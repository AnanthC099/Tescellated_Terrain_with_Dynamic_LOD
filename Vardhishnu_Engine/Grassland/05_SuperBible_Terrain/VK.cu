...
// OpenGL Superbible 7th Edition style terrain textures
typedef struct {
    VkImage image;
...
// Displacement map parameters (matches shader binding 2)
...
// Fog parameters (matches shader binding 4)
struct FogParams {
    int enable_fog;
    float fog_density;
    float fog_start;
...
// Texture handles
TextureData displacementTexture;
TextureData colorTexture;
VkSampler vkSampler = VK_NULL_HANDLE;

// Additional uniform buffers for displacement and fog
struct UniformData dispParamsUniform;
struct UniformData fogParamsUniform;

// Displacement and fog settings
struct DisplacementParams displacementParams = { 6.0f, 0.0f, 0.0f, 0.0f };
struct FogParams fogParams = { 1, 0.025f, 0.0f, 100.0f, glm::vec4(0.7f, 0.8f, 0.9f, 1.0f) };
...
VkResult initialize(void)
{
	...
	VkResult CreateSampler(void);
	VkResult CreateDisplacementTexture(void);
	VkResult CreateColorTexture(void);
	VkResult CreateAdditionalUniformBuffers(void);
	...
	// OpenGL Superbible 7th Edition style: Only 4 vertices needed (vertices generated in shader using gl_VertexIndex)
	unsigned int totalVertices = 4;
	...
	// OpenGL Superbible 7th Edition style texture and sampler creation
	vkResult = CreateSampler();
	if (vkResult != VK_SUCCESS)
	{
		FileIO("initialize(): CreateSampler() function failed with error code %d\n", vkResult);
		return vkResult;
	}
	else
	{
		FileIO("initialize(): CreateSampler() succedded\n");
	}

	vkResult = CreateDisplacementTexture();
	if (vkResult != VK_SUCCESS)
	{
		FileIO("initialize(): CreateDisplacementTexture() function failed with error code %d\n", vkResult);
		return vkResult;
	}
	else
	{
		FileIO("initialize(): CreateDisplacementTexture() succedded\n");
	}

	vkResult = CreateColorTexture();
	if (vkResult != VK_SUCCESS)
	{
		FileIO("initialize(): CreateColorTexture() function failed with error code %d\n", vkResult);
		return vkResult;
	}
	else
	{
		FileIO("initialize(): CreateColorTexture() succedded\n");
	}

	vkResult = CreateAdditionalUniformBuffers();
	if (vkResult != VK_SUCCESS)
	{
		FileIO("initialize(): CreateAdditionalUniformBuffers() function failed with error code %d\n", vkResult);
		return vkResult;
	}
	else
	{
		FileIO("initialize(): CreateAdditionalUniformBuffers() succedded\n");
	}
	...
	// OpenGL Superbible 7th Edition style: Sky blue background matching fog color
	vkClearColorValue.float32[0] = 0.7f;
	vkClearColorValue.float32[1] = 0.8f;
	vkClearColorValue.float32[2] = 0.9f;
...
VkResult display(void)
{
	...
	// OpenGL Superbible 7th Edition style: Vertices are generated in vertex shader using gl_InstanceIndex
	// No need for CUDA kernel - commented out for instance rendering approach
	/*
	dim3 block(8, 8, 1);
...
void uninitialize(void)
{
		...
		if(vkDevice)
		{
			...
			// OpenGL Superbible 7th Edition style cleanup - additional uniform buffers
			if(dispParamsUniform.vkBuffer)
			{
				vkDestroyBuffer(vkDevice, dispParamsUniform.vkBuffer, NULL);
				dispParamsUniform.vkBuffer = VK_NULL_HANDLE;
				FileIO("uninitialize(): dispParamsUniform.vkBuffer is freed\n");
			}
			if(dispParamsUniform.vkDeviceMemory)
			{
				vkFreeMemory(vkDevice, dispParamsUniform.vkDeviceMemory, NULL);
				dispParamsUniform.vkDeviceMemory = VK_NULL_HANDLE;
				FileIO("uninitialize(): dispParamsUniform.vkDeviceMemory is freed\n");
			}
			if(fogParamsUniform.vkBuffer)
			{
				vkDestroyBuffer(vkDevice, fogParamsUniform.vkBuffer, NULL);
				fogParamsUniform.vkBuffer = VK_NULL_HANDLE;
				FileIO("uninitialize(): fogParamsUniform.vkBuffer is freed\n");
			}
			if(fogParamsUniform.vkDeviceMemory)
			{
				vkFreeMemory(vkDevice, fogParamsUniform.vkDeviceMemory, NULL);
				fogParamsUniform.vkDeviceMemory = VK_NULL_HANDLE;
				FileIO("uninitialize(): fogParamsUniform.vkDeviceMemory is freed\n");
			}

			// OpenGL Superbible 7th Edition style cleanup - textures and sampler
			if(displacementTexture.view)
			{
				vkDestroyImageView(vkDevice, displacementTexture.view, NULL);
				displacementTexture.view = VK_NULL_HANDLE;
				FileIO("uninitialize(): displacementTexture.view is freed\n");
			}
			if(displacementTexture.memory)
			{
				vkFreeMemory(vkDevice, displacementTexture.memory, NULL);
				displacementTexture.memory = VK_NULL_HANDLE;
				FileIO("uninitialize(): displacementTexture.memory is freed\n");
			}
			if(displacementTexture.image)
			{
				vkDestroyImage(vkDevice, displacementTexture.image, NULL);
				displacementTexture.image = VK_NULL_HANDLE;
				FileIO("uninitialize(): displacementTexture.image is freed\n");
			}
			if(colorTexture.view)
			{
				vkDestroyImageView(vkDevice, colorTexture.view, NULL);
				colorTexture.view = VK_NULL_HANDLE;
				FileIO("uninitialize(): colorTexture.view is freed\n");
			}
			if(colorTexture.memory)
			{
				vkFreeMemory(vkDevice, colorTexture.memory, NULL);
				colorTexture.memory = VK_NULL_HANDLE;
				FileIO("uninitialize(): colorTexture.memory is freed\n");
			}
			if(colorTexture.image)
			{
				vkDestroyImage(vkDevice, colorTexture.image, NULL);
				colorTexture.image = VK_NULL_HANDLE;
				FileIO("uninitialize(): colorTexture.image is freed\n");
			}
			if(vkSampler)
			{
				vkDestroySampler(vkDevice, vkSampler, NULL);
				vkSampler = VK_NULL_HANDLE;
				FileIO("uninitialize(): vkSampler is freed\n");
			}
...
VkResult UpdateIndirectBuffer(void)
{
	...
	// OpenGL Superbible 7th Edition style: 4 vertices per patch, 4096 instances (64x64 grid)
	vkDrawIndirectCommand.vertexCount = 4;
	vkDrawIndirectCommand.instanceCount = PATCH_GRID_SIZE * PATCH_GRID_SIZE;  // 4096 instances
...
// OpenGL Superbible 7th Edition style helper function to find suitable memory type
...
// Perlin noise functions for heightmap generation
...
float noise2D(float x, float y) {
    ...
    float a = noiseHash(i + j * 57.0f);
    float b = noiseHash(i + 1.0f + j * 57.0f);
    float c = noiseHash(i + (j + 1.0f) * 57.0f);
    float d = noiseHash(i + 1.0f + (j + 1.0f) * 57.0f);
...
VkResult CreateSampler(void)
{
    ...
    vkResult = vkCreateSampler(vkDevice, &samplerInfo, NULL, &vkSampler);
    if (vkResult != VK_SUCCESS) {
        FileIO("CreateSampler(): vkCreateSampler() failed with error code %d\n", vkResult);
        return vkResult;
    }

    FileIO("CreateSampler(): vkCreateSampler() succeeded\n");
    return vkResult;
}

VkResult CreateTexture(TextureData* texture, unsigned char* data, int width, int height, VkFormat format)
{
    VkResult vkResult = VK_SUCCESS;
    ...
    vkResult = vkCreateImage(vkDevice, &imageInfo, NULL, &texture->image);
    if (vkResult != VK_SUCCESS) {
        FileIO("CreateTexture(): vkCreateImage() failed with error code %d\n", vkResult);
        return vkResult;
    }
    FileIO("CreateTexture(): vkCreateImage() succeeded\n");

    // Get memory requirements
    VkMemoryRequirements memRequirements;
    vkGetImageMemoryRequirements(vkDevice, texture->image, &memRequirements);
    ...
    if (allocInfo.memoryTypeIndex == UINT32_MAX) {
        FileIO("CreateTexture(): Failed to find suitable memory type\n");
        return VK_ERROR_INITIALIZATION_FAILED;
    }

    vkResult = vkAllocateMemory(vkDevice, &allocInfo, NULL, &texture->memory);
    if (vkResult != VK_SUCCESS) {
        FileIO("CreateTexture(): vkAllocateMemory() failed with error code %d\n", vkResult);
        return vkResult;
    }
    FileIO("CreateTexture(): vkAllocateMemory() succeeded\n");

    // Bind memory
    vkResult = vkBindImageMemory(vkDevice, texture->image, texture->memory, 0);
    if (vkResult != VK_SUCCESS) {
        FileIO("CreateTexture(): vkBindImageMemory() failed with error code %d\n", vkResult);
        return vkResult;
    }

    // Copy data to image
    void* mappedData;
    vkResult = vkMapMemory(vkDevice, texture->memory, 0, memRequirements.size, 0, &mappedData);
    if (vkResult != VK_SUCCESS) {
        FileIO("CreateTexture(): vkMapMemory() failed with error code %d\n", vkResult);
        return vkResult;
    }
    ...
    vkGetImageSubresourceLayout(vkDevice, texture->image, &subresource, &layout);

    int bytesPerPixel = (format == VK_FORMAT_R8G8B8A8_UNORM) ? 4 : 1;

    if (layout.rowPitch == (VkDeviceSize)(width * bytesPerPixel)) {
        memcpy(mappedData, data, width * height * bytesPerPixel);
    } else {
        unsigned char* dataPtr = (unsigned char*)mappedData;
        for (int y = 0; y < height; y++) {
            memcpy(dataPtr + y * layout.rowPitch, data + y * width * bytesPerPixel, width * bytesPerPixel);
        }
    }

    vkUnmapMemory(vkDevice, texture->memory);
    ...
    vkResult = vkAllocateCommandBuffers(vkDevice, &cmdBufAllocInfo, &cmdBuffer);
    if (vkResult != VK_SUCCESS) {
        FileIO("CreateTexture(): vkAllocateCommandBuffers() for layout transition failed\n");
        return vkResult;
    }
    ...
    FileIO("CreateTexture(): Image layout transition to SHADER_READ_ONLY_OPTIMAL succeeded\n");
    ...
    vkResult = vkCreateImageView(vkDevice, &viewInfo, NULL, &texture->view);
    if (vkResult != VK_SUCCESS) {
        FileIO("CreateTexture(): vkCreateImageView() failed with error code %d\n", vkResult);
        return vkResult;
    }
    FileIO("CreateTexture(): vkCreateImageView() succeeded\n");
...
VkResult CreateDisplacementTexture(void)
{
    ...
    // Generate heightmap using Perlin noise (similar to OpenGL Superbible heightmap)
    unsigned char* heightmapData = (unsigned char*)malloc(TEXTURE_SIZE * TEXTURE_SIZE);
    if (heightmapData == NULL) {
        FileIO("CreateDisplacementTexture(): malloc failed\n");
        return VK_ERROR_OUT_OF_HOST_MEMORY;
    }

    for (int y = 0; y < TEXTURE_SIZE; y++) {
        for (int x = 0; x < TEXTURE_SIZE; x++) {
            float fx = (float)x / (float)TEXTURE_SIZE;
            float fy = (float)y / (float)TEXTURE_SIZE;

            // Multi-octave noise for natural terrain
            float height = fbm2D(fx * 4.0f, fy * 4.0f, 6);

            // Normalize to 0-255 range
            height = (height + 0.5f) * 255.0f;
            height = fmaxf(0.0f, fminf(255.0f, height));

            heightmapData[y * TEXTURE_SIZE + x] = (unsigned char)height;
        }
    }

    vkResult = CreateTexture(&displacementTexture, heightmapData, TEXTURE_SIZE, TEXTURE_SIZE, VK_FORMAT_R8_UNORM);

    free(heightmapData);

    if (vkResult != VK_SUCCESS) {
        FileIO("CreateDisplacementTexture(): CreateTexture() failed\n");
        return vkResult;
    }

    FileIO("CreateDisplacementTexture(): Displacement texture created successfully\n");
    return vkResult;
}
...
VkResult CreateColorTexture(void)
{
    ...
    // Generate color texture (terrain colors based on height)
    unsigned char* colorData = (unsigned char*)malloc(TEXTURE_SIZE * TEXTURE_SIZE * 4);
    if (colorData == NULL) {
        FileIO("CreateColorTexture(): malloc failed\n");
        return VK_ERROR_OUT_OF_HOST_MEMORY;
    }
    ...
    for (int y = 0; y < TEXTURE_SIZE; y++) {
        for (int x = 0; x < TEXTURE_SIZE; x++) {
            ...
            // Get height for color determination
            float height = fbm2D(fx * 4.0f, fy * 4.0f, 6);
            height = (height + 0.5f);

            // Color based on height (similar to OpenGL Superbible landscape)
            unsigned char r, g, b;
            if (height < 0.3f) {
                // Deep water - blue
                r = 30; g = 60; b = 120;
    ...
    vkResult = CreateTexture(&colorTexture, colorData, TEXTURE_SIZE, TEXTURE_SIZE, VK_FORMAT_R8G8B8A8_UNORM);

    free(colorData);

    if (vkResult != VK_SUCCESS) {
        FileIO("CreateColorTexture(): CreateTexture() failed\n");
        return vkResult;
    }

    FileIO("CreateColorTexture(): Color texture created successfully\n");
    return vkResult;
}
...
VkResult CreateAdditionalUniformBuffers(void)
{
    ...
    // Create displacement params uniform buffer
    VkBufferCreateInfo bufferInfo;
    memset(&bufferInfo, 0, sizeof(VkBufferCreateInfo));
    bufferInfo.sType = VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO;
    bufferInfo.size = sizeof(struct DisplacementParams);
    bufferInfo.usage = VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT;
    bufferInfo.sharingMode = VK_SHARING_MODE_EXCLUSIVE;

    vkResult = vkCreateBuffer(vkDevice, &bufferInfo, NULL, &dispParamsUniform.vkBuffer);
    if (vkResult != VK_SUCCESS) {
        FileIO("CreateAdditionalUniformBuffers(): vkCreateBuffer() for disp params failed\n");
        return vkResult;
    }

    VkMemoryRequirements memReqs;
    vkGetBufferMemoryRequirements(vkDevice, dispParamsUniform.vkBuffer, &memReqs);
    ...
    vkResult = vkAllocateMemory(vkDevice, &allocInfo, NULL, &dispParamsUniform.vkDeviceMemory);
    if (vkResult != VK_SUCCESS) {
        FileIO("CreateAdditionalUniformBuffers(): vkAllocateMemory() for disp params failed\n");
        return vkResult;
    }

    vkBindBufferMemory(vkDevice, dispParamsUniform.vkBuffer, dispParamsUniform.vkDeviceMemory, 0);

    // Upload displacement params
    void* data;
    vkMapMemory(vkDevice, dispParamsUniform.vkDeviceMemory, 0, sizeof(struct DisplacementParams), 0, &data);
    memcpy(data, &displacementParams, sizeof(struct DisplacementParams));
    vkUnmapMemory(vkDevice, dispParamsUniform.vkDeviceMemory);

    // Create fog params uniform buffer
    bufferInfo.size = sizeof(struct FogParams);

    vkResult = vkCreateBuffer(vkDevice, &bufferInfo, NULL, &fogParamsUniform.vkBuffer);
    if (vkResult != VK_SUCCESS) {
        FileIO("CreateAdditionalUniformBuffers(): vkCreateBuffer() for fog params failed\n");
        return vkResult;
    }

    vkGetBufferMemoryRequirements(vkDevice, fogParamsUniform.vkBuffer, &memReqs);
    allocInfo.allocationSize = memReqs.size;
    allocInfo.memoryTypeIndex = findMemoryType(memReqs.memoryTypeBits,
        VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);

    vkResult = vkAllocateMemory(vkDevice, &allocInfo, NULL, &fogParamsUniform.vkDeviceMemory);
    if (vkResult != VK_SUCCESS) {
        FileIO("CreateAdditionalUniformBuffers(): vkAllocateMemory() for fog params failed\n");
        return vkResult;
    }

    vkBindBufferMemory(vkDevice, fogParamsUniform.vkBuffer, fogParamsUniform.vkDeviceMemory, 0);

    // Upload fog params
    vkMapMemory(vkDevice, fogParamsUniform.vkDeviceMemory, 0, sizeof(struct FogParams), 0, &data);
    memcpy(data, &fogParams, sizeof(struct FogParams));
    vkUnmapMemory(vkDevice, fogParamsUniform.vkDeviceMemory);

    FileIO("CreateAdditionalUniformBuffers(): Additional uniform buffers created successfully\n");
    return vkResult;
}
...
VkResult CreateDescriptorSetLayout()
{
	...
	// OpenGL Superbible 7th Edition style descriptor layout with 5 bindings:
	// 0: MVP uniform buffer
	// 1: Displacement texture sampler
	// 2: Displacement params uniform buffer
	// 3: Color texture sampler
	// 4: Fog params uniform buffer

	VkDescriptorSetLayoutBinding bindings[5];
	memset(bindings, 0, sizeof(bindings));

	// Binding 0: MVP matrices uniform buffer
	...
	// Binding 1: Displacement texture (heightmap)
	...
	// Binding 2: Displacement params uniform buffer
	...
	// Binding 3: Color texture (landscape)
	...
	// Binding 4: Fog params uniform buffer
...
VkResult CreateDescriptorPool(void)
{
	...
	// OpenGL Superbible 7th Edition style descriptor pool with:
	// - 3 uniform buffers (MVP, displacement params, fog params)
	// - 2 combined image samplers (displacement texture, color texture)

	VkDescriptorPoolSize poolSizes[2];
	memset(poolSizes, 0, sizeof(poolSizes));

	// Uniform buffers (3 total)
	poolSizes[0].type = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
	poolSizes[0].descriptorCount = 3;

	// Combined image samplers (2 total)
	...
	vkDescriptorPoolCreateInfo.pPoolSizes = poolSizes;
...
VkResult CreateDescriptorSet(void)
{
	...
	// OpenGL Superbible 7th Edition style descriptor set updates

	// Buffer infos for uniform buffers
	VkDescriptorBufferInfo bufferInfos[3];
	memset(bufferInfos, 0, sizeof(bufferInfos));

	// Binding 0: MVP uniform buffer
	bufferInfos[0].buffer = uniformData.vkBuffer;
	bufferInfos[0].offset = 0;
	bufferInfos[0].range = sizeof(struct MyUniformData);

	// Binding 2: Displacement params uniform buffer
	bufferInfos[1].buffer = dispParamsUniform.vkBuffer;
	bufferInfos[1].offset = 0;
	bufferInfos[1].range = sizeof(struct DisplacementParams);

	// Binding 4: Fog params uniform buffer
	bufferInfos[2].buffer = fogParamsUniform.vkBuffer;
	bufferInfos[2].offset = 0;
	bufferInfos[2].range = sizeof(struct FogParams);

	// Image infos for texture samplers
	VkDescriptorImageInfo imageInfos[2];
	memset(imageInfos, 0, sizeof(imageInfos));

	// Binding 1: Displacement texture
	imageInfos[0].sampler = vkSampler;
	imageInfos[0].imageView = displacementTexture.view;
	imageInfos[0].imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;

	// Binding 3: Color texture
	imageInfos[1].sampler = vkSampler;
	imageInfos[1].imageView = colorTexture.view;
	imageInfos[1].imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;

	// Write descriptor sets for all 5 bindings
	...
	writes[3].dstSet = vkDescriptorSet;
	writes[3].dstBinding = 3;
	writes[3].dstArrayElement = 0;
	writes[3].descriptorCount = 1;
	writes[3].descriptorType = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
	writes[3].pImageInfo = &imageInfos[1];

	// Binding 4: Fog params uniform buffer
	writes[4].sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
	writes[4].dstSet = vkDescriptorSet;
	...
	vkUpdateDescriptorSets(vkDevice, 5, writes, 0, NULL);

	FileIO("CreateDescriptorSet(): vkUpdateDescriptorSets() succedded for all 5 bindings\n");
...
VkResult CreatePipeline(void)
{
	...
	// OpenGL Superbible 7th Edition style: Solid filled polygons (use VK_POLYGON_MODE_LINE for wireframe)
