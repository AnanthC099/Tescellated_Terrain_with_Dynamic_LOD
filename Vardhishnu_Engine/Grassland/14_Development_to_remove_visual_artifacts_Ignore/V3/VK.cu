...
// Camera position for distance-based LOD (updated each frame)
float3 h_cameraPos = make_float3(0.0f, 50.0f, 0.0f);
__constant__ float3 d_cameraPos;
...
// Previous camera position for threshold-based updates
float3 h_prevCameraPos = make_float3(0.0f, 0.0f, 0.0f);
float cameraMovementThreshold = 0.5f;  // Only update if camera moves more than this
BOOL bForceHeightmapUpdate = TRUE;     // Force update on first frame
...
__global__ void performFrustumCulling(
    ...
    // Add margin for safety - larger margin prevents incorrect culling at grazing angles
    // The margin should be proportional to height scale to handle all terrain variations
    float heightMargin = heightScale * 0.15f;  // 15% of height range
    minH -= heightMargin;
    maxH += heightMargin;

    // Ensure minimum AABB height extent to avoid degenerate cases at flat areas
    float minExtent = heightScale * 0.1f;
...
VkResult display(void)
{
	...
	float3 currentCameraPos = make_float3(camPos.x, camPos.y, camPos.z);

	// Camera movement threshold check - only regenerate if camera moved significantly
	float cameraDist = sqrtf(
		(currentCameraPos.x - h_prevCameraPos.x) * (currentCameraPos.x - h_prevCameraPos.x) +
		(currentCameraPos.y - h_prevCameraPos.y) * (currentCameraPos.y - h_prevCameraPos.y) +
		(currentCameraPos.z - h_prevCameraPos.z) * (currentCameraPos.z - h_prevCameraPos.z)
	);
	BOOL bNeedHeightmapUpdate = (cameraDist > cameraMovementThreshold) || bForceHeightmapUpdate;

	if (bCudaTerrainSystemInitialized && heightmapData.d_heightmap && bNeedHeightmapUpdate) {
		...
		h_prevCameraPos = currentCameraPos;
	...
	if (bCudaTerrainSystemInitialized && d_frustumPlanes && d_visibilityMask) {
		// Extract frustum planes from view-projection matrix
		// Use non-Vulkan projection to avoid Y-flip which inverts top/bottom plane normals
		glm::mat4 viewProj = g_camera.getProjectionMatrix() * g_camera.getViewMatrix();
