...
// Soil heterogeneity mask - rocky vs sandy vs clay areas
float getSoilHeterogeneityMask(vec2 p) {
    // Create patchy soil variation zones
    float coarse = fbmRotated(p * 0.012 + vec2(777.0, 333.0), 3);
    float fine = noise(p * 0.025 + vec2(111.0, 222.0));
...
float getPebbleHeight(vec2 worldPos) {
    ...
    for (int y = -1; y <= 1; y++) {
        for (int x = -1; x <= 1; x++) {
            vec2 neighbor = vec2(float(x), float(y));
            vec3 randVal = hash3(stoneCell + neighbor);
    ...
    for (int y = -1; y <= 1; y++) {
        for (int x = -1; x <= 1; x++) {
            vec2 neighbor = vec2(float(x), float(y));
            vec3 randVal = hash3(pebbleCell + neighbor + vec2(500.0, 500.0));
...
float getMicroErosionHeight(vec2 worldPos) {
    ...
    // Primary micro-channels using cellular noise
    vec2 erosionCoord = worldPos * 0.04;
    vec2 warpedCoord = domainWarpLight(erosionCoord, 0.3);
    float channels = cellularNoise(warpedCoord * 2.0);
    ...
    float fineChannels = cellularNoise(fineCoord * 3.0 + vec2(100.0, 200.0));
    erosion -= smoothstep(0.08, 0.0, fineChannels) * 0.008;

    // Branching erosion pattern
    float branchNoise = noise(worldPos * 0.06 + vec2(50.0, 75.0));
...
float getHighFrequencyBumps(vec2 worldPos, float heterogeneityMask) {
    ...
    // Very high frequency noise for soil texture
    float freq1 = noise(worldPos * 0.12) * 0.008;
    float freq2 = noise(worldPos * 0.25 + vec2(33.0, 77.0)) * 0.005;
    float freq3 = simplexNoise(worldPos * 0.4 + vec2(99.0, 11.0)) * 0.003;
    float freq4 = noise(worldPos * 0.6 + vec2(55.0, 44.0)) * 0.002;
    ...
    // Add occasional larger soil clumps
    float clumpNoise = fbmRotated(worldPos * 0.02 + vec2(123.0, 456.0), 2);
    float clumps = smoothstep(0.3, 0.5, clumpNoise) * noise(worldPos * 0.08) * 0.012;
...
float getRootBumps(vec2 worldPos) {
    ...
    // Main root directions (radial from imaginary tree positions)
    float rootPattern1 = sin(rootCoord.x * 5.0 + noise(rootCoord * 2.0) * 4.0);
    float rootPattern2 = sin(rootCoord.y * 4.5 + noise(rootCoord * 2.5 + vec2(10.0, 20.0)) * 3.5);
    ...
    // Sparse placement using mask
    float rootMask = smoothstep(0.2, 0.5, fbmRotated(worldPos * 0.008 + vec2(200.0, 300.0), 2));
    roots = (ridge1 + ridge2) * rootMask;

    return roots;
}

// Footprint/compression zones - subtle depressions
float getCompressionDepth(vec2 worldPos) {
    ...
    for (int y = -1; y <= 1; y++) {
        for (int x = -1; x <= 1; x++) {
            vec2 neighbor = vec2(float(x), float(y));
            vec3 randVal = hash3(compCell + neighbor + vec2(999.0, 888.0));
