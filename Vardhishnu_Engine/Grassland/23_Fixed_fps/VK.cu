...
// Per-frame-in-flight semaphores to avoid validation layer errors
// VUID-vkAcquireNextImageKHR-semaphore-01779 requires semaphores not have pending operations
#define MAX_FRAMES_IN_FLIGHT 3
VkSemaphore vkSemaphore_ImageAvailable[MAX_FRAMES_IN_FLIGHT] = {VK_NULL_HANDLE};
VkSemaphore vkSemaphore_RenderComplete[MAX_FRAMES_IN_FLIGHT] = {VK_NULL_HANDLE};
uint32_t currentFrame = 0;

VkFence *vkFence_array = NULL;

// Track which fence is associated with each swapchain image
// Needed to wait for the correct fence before reusing a command buffer
VkFence *imageInFlightFences = NULL;
...
// Flag for one-time terrain patch generation
BOOL bTerrainPatchesGenerated = FALSE;

// Flag for command buffer caching
BOOL bCommandBuffersBuilt = FALSE;
...
__global__ void generateGradientTable(float4* gradients, curandState* states, int tableSize) {
    ...
    if (idx < tableSize) {
        ...
        float cosVal = fminf(fmaxf(2.0f * u2 - 1.0f, -1.0f), 1.0f);
        float phi = acosf(cosVal);
...
// Smoothstep function (with safeguard against edge0 == edge1)
__device__ __forceinline__ float smoothstep_cuda(float edge0, float edge1, float x) {
    ...
    if (fabsf(range) < 0.0001f) {
        return (x < edge0) ? 0.0f : 1.0f;  // Step function when edges are equal
    }
    float t = fminf(fmaxf((x - edge0) / range, 0.0f), 1.0f);
...
__device__ float noise2D(float2 p, const float4* __restrict__ gradTable,
    ...
    // Use 12-bit masking (4096) instead of 8-bit (256) to avoid visible tiling
    // 4096-unit periodicity >> 800-unit terrain size eliminates repetition
    int ix = (int)floorf(p.x) & 4095;
    int iy = (int)floorf(p.y) & 4095;
...
__device__ float noise2DSeeded(float2 p, float seed, const float4* __restrict__ gradTable,
                                const int* __restrict__ permTable) {
    // Use 12-bit masking (4096) instead of 8-bit (256) to avoid visible tiling
    int seedOffset = (int)(seed * 17.31f) & 4095;
    p.x += seed * 7.31f;
    p.y += seed * 11.17f;

    int ix = ((int)floorf(p.x) + seedOffset) & 4095;
    int iy = ((int)floorf(p.y) + (int)(seed * 23.57f)) & 4095;
    float fx = p.x - floorf(p.x);
    float fy = p.y - floorf(p.y);
    ...
    // Use 12-bit masking for more variation
    int gradOffset = (int)(seed * 7.13f) & 4095;
...
__device__ float calculateTerrainHeight(float2 worldPos, float3 cameraPos,
                                         const float4* gradTable, const int* permTable) {
    // =========================================================================
    // NATURAL GRASSLAND TERRAIN
    // Organically shaped rolling hills with irregular micro-details
    // Enhanced with macro hills, clumpy micro-noise, patch masks, and drainage
    // =========================================================================

    float height = 0.0f;

    // =========================================================================
    // DOMAIN WARPING - Multiple scales for organic, non-uniform terrain
    // =========================================================================

    // Primary domain warp (large scale) - organic macro terrain shapes
    float2 warpCoord = make_float2(worldPos.x * 0.002f, worldPos.y * 0.002f);
    float warpX = noise2DSeeded(warpCoord, 73.0f, gradTable, permTable) * 25.0f;
    float warpY = noise2DSeeded(make_float2(warpCoord.y + 31.5f, warpCoord.x - 17.3f), 89.0f, gradTable, permTable) * 25.0f;
    float2 warpedPos = make_float2(worldPos.x + warpX, worldPos.y + warpY);

    // Secondary domain warp (medium scale) - reduces uniformity in mid-features
    float2 warpCoord2 = make_float2(worldPos.x * 0.008f, worldPos.y * 0.008f);
    float warpX2 = noise2DSeeded(warpCoord2, 251.0f, gradTable, permTable) * 8.0f;
    float warpY2 = noise2DSeeded(make_float2(warpCoord2.y + 19.7f, warpCoord2.x - 23.1f), 257.0f, gradTable, permTable) * 8.0f;
    float2 warpedPos2 = make_float2(worldPos.x + warpX2, worldPos.y + warpY2);

    // Tertiary domain warp (fine scale) - reduces uniformity in micro-features
    float2 warpCoord3 = make_float2(worldPos.x * 0.025f, worldPos.y * 0.025f);
    float warpX3 = noise2DSeeded(warpCoord3, 263.0f, gradTable, permTable) * 3.0f;
    float warpY3 = noise2DSeeded(make_float2(warpCoord3.y + 13.3f, warpCoord3.x - 11.7f), 269.0f, gradTable, permTable) * 3.0f;
    float2 warpedPos3 = make_float2(worldPos.x + warpX3, worldPos.y + warpY3);

    // =========================================================================
    // MACRO HILLS - Very low frequency landscape-scale features
    // Creates the broad, sweeping hills visible from far away
    // =========================================================================

    // Primary macro hills - very large scale rolling landscape
    float2 macroCoord = make_float2(warpedPos.x * 0.0008f, warpedPos.y * 0.0008f);
    float macroHills = fbmRotatedSeeded(macroCoord, 3, 277.0f, gradTable, permTable);
    height += macroHills * 0.55f;

    // Secondary macro undulation - slightly higher frequency for variety
    float2 macroCoord2 = make_float2(warpedPos.x * 0.0015f, warpedPos.y * 0.0012f);
    float macroUndulation = fbmRotatedSeeded(macroCoord2, 3, 281.0f, gradTable, permTable);
    height += macroUndulation * 0.35f;

    // =========================================================================
    // PATCH MASKS - Control roughness variation across terrain
    // Some areas are smooth, others have more micro-detail
    // =========================================================================

    // Master roughness mask - large areas of varying texture intensity
    float2 roughMaskCoord = make_float2(warpedPos.x * 0.004f, warpedPos.y * 0.0045f);
    float roughMaskNoise = fbmRotatedSeeded(roughMaskCoord, 2, 283.0f, gradTable, permTable);
    float roughnessMask = smoothstep_cuda(-0.4f, 0.6f, roughMaskNoise);

    // Secondary roughness variation - medium scale patches
    float2 roughMaskCoord2 = make_float2(warpedPos2.x * 0.015f, warpedPos2.y * 0.018f);
    float roughMaskNoise2 = noise2DSeeded(roughMaskCoord2, 293.0f, gradTable, permTable);
    float roughnessMask2 = smoothstep_cuda(-0.2f, 0.5f, roughMaskNoise2);

    // Combined roughness factor (0.3 to 1.0 range - always some texture)
    float combinedRoughness = 0.3f + 0.7f * roughnessMask * (0.5f + 0.5f * roughnessMask2);

    // =========================================================================
    // PRIMARY TERRAIN FEATURES
    // =========================================================================

    // Primary rolling hills with warped coordinates for irregular shapes
    float2 hillCoord = make_float2(warpedPos.x * 0.003f, warpedPos.y * 0.003f);
    float rollingHills = fbmRotatedSeeded(hillCoord, 4, 101.0f, gradTable, permTable);
    height += rollingHills * 0.40f;

    // Asymmetric secondary undulations - different frequencies per axis
    float2 undulationCoord = make_float2(warpedPos.x * 0.006f, warpedPos.y * 0.009f);
    float undulations = fbmRotatedSeeded(undulationCoord, 3, 127.0f, gradTable, permTable);
    height += undulations * 0.20f;

    // Subtle ridge-like features running in varied directions
    float2 ridgeCoord = make_float2(warpedPos2.x * 0.012f + warpedPos2.y * 0.004f,
                                     warpedPos2.y * 0.011f - warpedPos2.x * 0.003f);
    float ridgeNoise = noise2DSeeded(ridgeCoord, 151.0f, gradTable, permTable);
    float ridge = fabsf(ridgeNoise) * 0.7f + 0.3f;
    height += (1.0f - ridge) * 0.07f;

    // =========================================================================
    // DRAINAGE CHANNELS - Multiple shallow gullies at different scales
    // =========================================================================

    // Primary drainage channel - main shallow gullies
    float2 channelCoord = make_float2(warpedPos.x * 0.008f, warpedPos.y * 0.015f);
    float channelNoise = noise2DSeeded(channelCoord, 163.0f, gradTable, permTable);
    float channel = 1.0f - fabsf(channelNoise);
    channel = channel * channel * channel;
    height -= channel * 0.055f;

    // Secondary drainage - perpendicular orientation, finer scale
    float2 channelCoord2 = make_float2(warpedPos.x * 0.018f + warpedPos.y * 0.006f,
                                        warpedPos.y * 0.005f - warpedPos.x * 0.016f);
    float channelNoise2 = noise2DSeeded(channelCoord2, 307.0f, gradTable, permTable);
    float channel2 = 1.0f - fabsf(channelNoise2);
    channel2 = channel2 * channel2 * channel2;
    height -= channel2 * 0.035f;

    // Tertiary drainage - very subtle, meandering micro-channels
    float2 channelCoord3 = make_float2(warpedPos2.x * 0.035f, warpedPos2.y * 0.028f);
    float channelNoise3 = noise2DSeeded(channelCoord3, 311.0f, gradTable, permTable);
    float channel3 = 1.0f - fabsf(channelNoise3);
    channel3 = channel3 * channel3;
    height -= channel3 * 0.02f * combinedRoughness;

    // =========================================================================
    // MEDIUM-SCALE FEATURES (modulated by roughness mask)
    // =========================================================================

    // Medium-scale terrain variation for natural undulation
    float2 medCoord = make_float2(warpedPos2.x * 0.018f, warpedPos2.y * 0.022f);
    float medVariation = fbmRotatedSeeded(medCoord, 2, 137.0f, gradTable, permTable);
    height += medVariation * 0.09f * combinedRoughness;

    // Fine-scale grass clump variation
    float2 grassCoord = make_float2(warpedPos2.x * 0.035f, warpedPos2.y * 0.04f);
    float grassVariation = noise2DSeeded(grassCoord, 149.0f, gradTable, permTable);
    height += grassVariation * 0.035f * combinedRoughness;

    // Micro-scale soil irregularities - small bumps and depressions
    float2 microCoord = make_float2(warpedPos3.x * 0.08f, warpedPos3.y * 0.075f);
    float microBumps = noise2DSeeded(microCoord, 173.0f, gradTable, permTable);
    height += microBumps * 0.016f * combinedRoughness;

    // Very fine detail - ground texture variation
    float2 fineCoord = make_float2(warpedPos3.x * 0.15f, warpedPos3.y * 0.16f);
    float fineDetail = noise2DSeeded(fineCoord, 191.0f, gradTable, permTable);
    height += fineDetail * 0.007f * combinedRoughness;

    // Scattered small mounds (animal burrows, roots, etc.)
    float2 moundCoord = make_float2(warpedPos2.x * 0.025f, warpedPos2.y * 0.028f);
    float moundNoise = noise2DSeeded(moundCoord, 199.0f, gradTable, permTable);
    float mound = fmaxf(0.0f, moundNoise - 0.6f) * 2.5f;
    height += mound * mound * 0.025f * combinedRoughness;

    // =========================================================================
    // CLUMPY MICRO-NOISE - High frequency grouped into patches
    // Creates clusters of fine detail rather than uniform noise
    // =========================================================================

    // Clump mask - defines where micro-detail clusters appear
    float2 clumpMaskCoord = make_float2(warpedPos2.x * 0.05f, warpedPos2.y * 0.055f);
    float clumpMaskNoise = noise2DSeeded(clumpMaskCoord, 313.0f, gradTable, permTable);
    float clumpMask = smoothstep_cuda(-0.1f, 0.6f, clumpMaskNoise);
    clumpMask = clumpMask * clumpMask;  // Sharper falloff for distinct patches

    // High-frequency noise modulated by clump mask
    float2 clumpyMicroCoord = make_float2(warpedPos3.x * 0.4f, warpedPos3.y * 0.38f);
    float clumpyMicro = noise2DSeeded(clumpyMicroCoord, 317.0f, gradTable, permTable);
    height += clumpyMicro * 0.022f * clumpMask * combinedRoughness;

    // Secondary clumpy layer - different frequency and offset
    float2 clumpyMicroCoord2 = make_float2(warpedPos3.x * 0.65f, warpedPos3.y * 0.6f);
    float clumpyMicro2 = noise2DSeeded(clumpyMicroCoord2, 331.0f, gradTable, permTable);
    height += clumpyMicro2 * 0.015f * clumpMask * combinedRoughness;

    // =========================================================================
    // GRASS SURFACE TEXTURE - Realistic uneven grass surface
    // Adds the bumpy, clumpy texture of real grassland
    // =========================================================================

    // Grass tuft clusters - primary surface roughness
    float2 tuftCoord = make_float2(warpedPos3.x * 0.5f, warpedPos3.y * 0.55f);
    float tuftNoise = noise2DSeeded(tuftCoord, 211.0f, gradTable, permTable);
    float tuft = fmaxf(0.0f, tuftNoise) * tuftNoise;
    height += tuft * 0.03f * combinedRoughness;

    // Secondary grass clumps at different scale
    float2 clumpCoord = make_float2(warpedPos3.x * 0.8f, warpedPos3.y * 0.75f);
    float clumpNoise = noise2DSeeded(clumpCoord, 223.0f, gradTable, permTable);
    float clump = fmaxf(0.0f, clumpNoise - 0.2f);
    height += clump * clump * 0.022f * combinedRoughness;

    // Fine grass blade variation - very high frequency bumps
    float2 bladeCoord = make_float2(warpedPos3.x * 1.5f, warpedPos3.y * 1.4f);
    float bladeNoise = noise2DSeeded(bladeCoord, 227.0f, gradTable, permTable);
    height += bladeNoise * 0.01f * combinedRoughness;

    // Ultra-fine surface granularity - soil and root texture
    float2 grainCoord = make_float2(warpedPos3.x * 2.5f, warpedPos3.y * 2.3f);
    float grainNoise = noise2DSeeded(grainCoord, 229.0f, gradTable, permTable);
    height += grainNoise * 0.005f * combinedRoughness;

    // Patchy grass density - some areas thicker than others
    float2 patchCoord = make_float2(warpedPos2.x * 0.12f, warpedPos2.y * 0.14f);
    float patchNoise = noise2DSeeded(patchCoord, 233.0f, gradTable, permTable);
    float patchMask = smoothstep_cuda(-0.3f, 0.5f, patchNoise);

    // Apply extra bumps in thick grass patches
    float2 thickCoord = make_float2(warpedPos3.x * 0.6f, warpedPos3.y * 0.65f);
    float thickNoise = noise2DSeeded(thickCoord, 239.0f, gradTable, permTable);
    height += thickNoise * 0.018f * patchMask * combinedRoughness;

    // Small depressions between grass clumps
    float2 gapCoord = make_float2(warpedPos3.x * 0.4f, warpedPos3.y * 0.38f);
    float gapNoise = noise2DSeeded(gapCoord, 241.0f, gradTable, permTable);
    float gap = fminf(0.0f, gapNoise + 0.3f);
    height += gap * 0.015f * combinedRoughness;
...
__global__ void generateTerrainHeightmap(
    ...
    // Safeguard against NaN/Inf from accumulated floating-point operations
    // These can cause black spots in rendering if not caught here
    if (isnan(height) || isinf(height)) {
        height = 0.0f;  // Default to flat terrain for invalid values
    }

    // Normalize height to 0-1 range for shader material selection
    // Raw heights range approximately -0.5 to +1.8, normalize to 0-1
    height = (height + 0.5f) / 2.3f;
    height = fmaxf(0.0f, fminf(1.0f, height));  // Clamp to valid range
...
__global__ void computeNormalMap(
    ...
    // Safeguard against NaN/Inf in height samples (prevents black spots in normals)
    if (isnan(hL) || isinf(hL)) hL = 0.0f;
    if (isnan(hR) || isinf(hR)) hR = 0.0f;
    if (isnan(hD) || isinf(hD)) hD = 0.0f;
    if (isnan(hU) || isinf(hU)) hU = 0.0f;

    // Compute world-space step size (protected against division by zero)
    float stepSize = (heightmapSize > 1) ? terrainSize / (float)(heightmapSize - 1) : 1.0f;
    ...
    // Normalize (with safeguard against NaN from invalid heightmap data)
    ...
    if (len > 0.0001f) {
        ...
        // Default to up-facing normal if length is zero (avoids NaN black spots)
    ...
    // Final safeguard: validate all components before storing
    if (isnan(slope) || isinf(slope)) slope = 0.0f;
    if (isnan(normal.x) || isinf(normal.x)) normal.x = 0.0f;
    if (isnan(normal.y) || isinf(normal.y)) normal.y = 1.0f;
    if (isnan(normal.z) || isinf(normal.z)) normal.z = 0.0f;
...
__global__ void computeTangentSpace(
    ...
    // Safeguard against NaN in normal data (prevents black spots)
    if (isnan(N.x) || isinf(N.x) || isnan(N.y) || isinf(N.y) || isnan(N.z) || isinf(N.z)) {
        N = make_float3(0.0f, 1.0f, 0.0f);  // Default to up-facing normal
    }
    ...
    if (lenT > 0.0001f) {
        ...
        // Default tangent when normal is nearly vertical (avoids NaN in shader)
    ...
    // Final safeguard: validate tangent components before storing
    if (isnan(T.x) || isinf(T.x)) T.x = 1.0f;
    if (isnan(T.y) || isinf(T.y)) T.y = 0.0f;
    if (isnan(T.z) || isinf(T.z)) T.z = 0.0f;
    if (isnan(sign) || isinf(sign)) sign = 1.0f;
...
void extractFrustumPlanes(float4* planes, const glm::mat4& viewProj) {
    ...
    // Normalize all planes (with safeguard against degenerate planes)
    for (int i = 0; i < 6; i++) {
        ...
        // If len is zero, leave plane unnormalized (will cull nothing, avoiding NaN)
    }
}
...
VkResult resize(int width, int height)
{
	...
	// Reset image-to-fence tracking for new swapchain images
	// vkDeviceWaitIdle() above ensures no fences are pending
	if(imageInFlightFences)
	{
		free(imageInFlightFences);
	}
	imageInFlightFences = (VkFence*)malloc(sizeof(VkFence) * swapchainImageCount);
	for(uint32_t i = 0; i < swapchainImageCount; i++)
	{
		imageInFlightFences[i] = VK_NULL_HANDLE;
	}
	FileIO("resize(): imageInFlightFences reset for %d swapchain images\n", swapchainImageCount);
...
VkResult display(void)
{
	...
	if (bCudaTerrainSystemInitialized && heightmapData.d_heightmap && bForceHeightmapUpdate) {
		...
		// Synchronize before copying static data to Vulkan buffers
		cudaDeviceSynchronize();

		// Copy heightmap data to Vulkan storage buffer (one-time, static data)
		if (noiseStorageBuffers.heightmapBuffer && heightmapData.d_heightmap) {
			...
			if (vkResult == VK_SUCCESS) {
				cudaMemcpy(heightmapPtr, heightmapData.d_heightmap, heightmapSize, cudaMemcpyDeviceToHost);
				vkUnmapMemory(vkDevice, noiseStorageBuffers.heightmapMemory);
			}
		}

		// Copy normal map data to Vulkan storage buffer (one-time, static data)
		if (noiseStorageBuffers.normalBuffer && d_normalMap) {
			void* normalPtr = NULL;
			...
			if (vkResult == VK_SUCCESS) {
				cudaMemcpy(normalPtr, d_normalMap, normalSize, cudaMemcpyDeviceToHost);
				vkUnmapMemory(vkDevice, noiseStorageBuffers.normalMemory);
			}
		}
	...
	// Generate flat terrain patches (base grid) - one-time operation
	if (!bTerrainPatchesGenerated) {
		dim3 block(8, 8, 1);
		...
		bTerrainPatchesGenerated = TRUE;
	}

	// Copy tessellation factors to Vulkan storage buffer (per-frame, camera-dependent)
	...
	// Build command buffers once - they use indirect draw which reads from GPU buffer
	if (!bCommandBuffersBuilt) {
		vkResult = buildCommandBuffers();
		if (vkResult != VK_SUCCESS)
		{
		...
		bCommandBuffersBuilt = TRUE;
	}

	// Wait for the in-flight fence BEFORE acquiring to ensure semaphore is not pending
	// This fixes VUID-vkAcquireNextImageKHR-semaphore-01779
	vkResult = vkWaitForFences(vkDevice, 1, &vkFence_array[currentFrame], VK_TRUE, UINT64_MAX);
	if(vkResult != VK_SUCCESS)
	{
	...
	vkResult = vkAcquireNextImageKHR(vkDevice, vkSwapchainKHR, UINT64_MAX, vkSemaphore_ImageAvailable[currentFrame], VK_NULL_HANDLE, &currentImageIndex);
	if(vkResult != VK_SUCCESS)
	{
	...
	// Wait for the fence that was last used with this swapchain image
	// This ensures the command buffer for this image is no longer in use
	// Fixes VUID-vkQueueSubmit-pCommandBuffers-00071
	if(imageInFlightFences[currentImageIndex] != VK_NULL_HANDLE)
	{
		vkResult = vkWaitForFences(vkDevice, 1, &imageInFlightFences[currentImageIndex], VK_TRUE, UINT64_MAX);
		if(vkResult != VK_SUCCESS)
		{
			FileIO("display(): vkWaitForFences() for image fence failed\n");
			return vkResult;
		}
	}

	vkResult = vkResetFences(vkDevice, 1, &vkFence_array[currentFrame]);
	if(vkResult != VK_SUCCESS)
	{
	...
	vkSubmitInfo.pWaitSemaphores = &vkSemaphore_ImageAvailable[currentFrame];
	vkSubmitInfo.commandBufferCount = 1;
	vkSubmitInfo.pCommandBuffers = &vkCommandBuffer_array[currentImageIndex];
	vkSubmitInfo.signalSemaphoreCount = 1;
	vkSubmitInfo.pSignalSemaphores = &vkSemaphore_RenderComplete[currentFrame];

	vkResult = vkQueueSubmit(vkQueue, 1, &vkSubmitInfo, vkFence_array[currentFrame]);
	if(vkResult != VK_SUCCESS)
	{
	...
	// Record that this fence is now associated with this swapchain image
	imageInFlightFences[currentImageIndex] = vkFence_array[currentFrame];
	...
	vkPresentInfoKHR.pWaitSemaphores = &vkSemaphore_RenderComplete[currentFrame];
...
void uninitialize(void)
{
		...
		if(vkDevice)
		{
			...
			// Free image-to-fence tracking array
			if(imageInFlightFences)
			{
				free(imageInFlightFences);
				imageInFlightFences = NULL;
				FileIO("uninitialize(): imageInFlightFences is freed\n");
			}

			// Destroy per-frame-in-flight semaphores
			for(uint32_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++)
			{
				if(vkSemaphore_RenderComplete[i])
				{
					...
					FileIO("uninitialize(): vkSemaphore_RenderComplete[%d] is freed\n", i);
				}

				if(vkSemaphore_ImageAvailable[i])
				{
					vkDestroySemaphore(vkDevice, vkSemaphore_ImageAvailable[i], NULL);
					vkSemaphore_ImageAvailable[i] = VK_NULL_HANDLE;
					FileIO("uninitialize(): vkSemaphore_ImageAvailable[%d] is freed\n", i);
				}
			}
...
VkResult CreateSemaphores(void)
{
	...
	// Create per-frame-in-flight semaphores to avoid VUID-vkAcquireNextImageKHR-semaphore-01779
	// Each frame in flight needs its own semaphore pair to prevent pending operations conflict
	for (uint32_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++)
	{
		vkResult = vkCreateSemaphore(vkDevice, &vkSemaphoreCreateInfo, NULL, &vkSemaphore_ImageAvailable[i]);
		if (vkResult != VK_SUCCESS)
		{
			FileIO("CreateSemaphores(): vkCreateSemaphore() function failed with error code %d for vkSemaphore_ImageAvailable[%d]\n", vkResult, i);
			return vkResult;
		}
		else
		{
			FileIO("CreateSemaphores(): vkCreateSemaphore() succedded for vkSemaphore_ImageAvailable[%d]\n", i);
		}

		vkResult = vkCreateSemaphore(vkDevice, &vkSemaphoreCreateInfo, NULL, &vkSemaphore_RenderComplete[i]);
		if (vkResult != VK_SUCCESS)
		{
			FileIO("CreateSemaphores(): vkCreateSemaphore() function failed with error code %d for vkSemaphore_RenderComplete[%d]\n", vkResult, i);
			return vkResult;
		}
		else
		{
			FileIO("CreateSemaphores(): vkCreateSemaphore() succedded for vkSemaphore_RenderComplete[%d]\n", i);
		}
	}
...
VkResult CreateFences(void)
{
	...
	// Create per-frame-in-flight fences to synchronize with per-frame semaphores
	vkFence_array = (VkFence*)malloc(sizeof(VkFence) * MAX_FRAMES_IN_FLIGHT);
	...
	// Allocate and initialize image-to-fence tracking array
	// This tracks which fence was last used with each swapchain image
	imageInFlightFences = (VkFence*)malloc(sizeof(VkFence) * swapchainImageCount);
	for(uint32_t i = 0; i < swapchainImageCount; i++)
	{
		imageInFlightFences[i] = VK_NULL_HANDLE;
	}
	FileIO("CreateFences(): imageInFlightFences array initialized for %d swapchain images\n", swapchainImageCount);
