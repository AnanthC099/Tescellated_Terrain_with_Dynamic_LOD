...
VkResult CreateCloudParamsUniformBuffer(void)
{
	...
	// Camera positioned to see multiple scattered clouds across the sky
	glm::vec3 fixedPosition = glm::vec3(0.0f, 3.0f, 50.0f);
	float upwardTilt = glm::radians(25.0f);  // 25 degrees upward tilt to see clouds at various heights
	...
	float fixedFov = 75.0f;  // Wider FOV to see more of the scattered cloudscape
...
VkResult CreateComputeDescriptorSetLayout(void)
{
	...
	// Binding 2: Acceleration structure (for ray tracing - optional)
	...
	// Ray tracing acceleration structure (binding 2) - only if RT supported
	if (bRayTracingSupported)
	{
...
VkResult CreateComputeDescriptorPool(void)
{
	...
	// Pool sizes for: storage image (output), uniform buffer
...
VkResult CreateComputeDescriptorSet(void)
{
	...
	fprintf(gFILE, "CreateComputeDescriptorSet(): Descriptor set updated\n");
...
VkResult UpdateComputeDescriptorSetWithAccelerationStructure(void)
{
	...
	// Write acceleration structure to descriptor set at binding 2
...
VkResult UpdateDescriptorSetsForNewStorageImage(void)
{
	...
	memset(&computeDescriptorWrite, 0, sizeof(VkWriteDescriptorSet));

	// Storage image (binding 0)
	computeDescriptorWrite.sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
	computeDescriptorWrite.dstSet = vkDescriptorSet_compute;
	computeDescriptorWrite.dstBinding = 0;
	computeDescriptorWrite.dstArrayElement = 0;
	computeDescriptorWrite.descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_IMAGE;
	computeDescriptorWrite.descriptorCount = 1;
	computeDescriptorWrite.pImageInfo = &computeImageInfo;

	vkUpdateDescriptorSets(vkDevice, 1, &computeDescriptorWrite, 0, NULL);
	fprintf(gFILE, "UpdateDescriptorSetsForNewStorageImage(): Compute descriptor set updated\n");
