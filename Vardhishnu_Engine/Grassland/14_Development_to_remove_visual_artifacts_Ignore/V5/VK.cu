...
VkResult UpdateUniformBuffer(void)
{
	...
	// Static view matrix - looking at origin from position (0, 30, 150)
	glm::vec3 viewPosition = glm::vec3(0.0f, 30.0f, 150.0f);
	glm::vec3 viewTarget = glm::vec3(0.0f, 0.0f, 0.0f);
	glm::vec3 viewUp = glm::vec3(0.0f, 1.0f, 0.0f);
	myUniformData.viewMatrix = glm::lookAt(viewPosition, viewTarget, viewUp);
...
VkResult display(void)
{
	...
	if (bCudaTerrainSystemInitialized && d_frustumPlanes && d_visibilityMask) {
		// Extract frustum planes from view-projection matrix
		// Use static view and projection matrices
		glm::vec3 viewPosition = glm::vec3(0.0f, 30.0f, 150.0f);
		glm::vec3 viewTarget = glm::vec3(0.0f, 0.0f, 0.0f);
		glm::vec3 viewUp = glm::vec3(0.0f, 1.0f, 0.0f);
		glm::mat4 staticView = glm::lookAt(viewPosition, viewTarget, viewUp);
		float aspectRatio = (float)winWidth / (float)winHeight;
		glm::mat4 staticProj = glm::perspective(glm::radians(45.0f), aspectRatio, 0.1f, 2000.0f);
		glm::mat4 viewProj = staticProj * staticView;
