...
*/
const char* enabledDeviceExtensionNames_array[2]; //VK_KHR_EXTERNAL_MEMORY_WIN32_EXTENSION_NAME
...
float cameraZ = 6.0f;      // Camera Z position (starts back from center)
float cameraSpeed = 0.5f;  // Movement speed per key press
...
// Constants for terrain patch grid
#define PATCH_GRID_SIZE 64  // 64x64 patches = 64*64*4 = 16384 vertices
#define TERRAIN_SIZE 8.0f   // 8 units terrain size

// Flat terrain patch generator kernel for dynamic tessellation LOD
// Generates vertices for quad patches (4 vertices per patch)
// Each thread generates one patch (4 vertices)
__global__ void generateFlatTerrainPatches(float4 *pos, unsigned int patchGridSize,
  ...
  // Calculate the base index for this patch's 4 vertices
  unsigned int baseIdx = (patchY * patchGridSize + patchX) * 4;

  // Calculate patch corner coordinates
  ...
  // Generate 4 corner vertices for this quad patch (counter-clockwise)
  // Terrain in x-z plane with y=0
  pos[baseIdx + 0] = make_float4(x0, 0.0f, z0, 1.0f);  // Bottom-left
  pos[baseIdx + 1] = make_float4(x1, 0.0f, z0, 1.0f);  // Bottom-right
  pos[baseIdx + 2] = make_float4(x1, 0.0f, z1, 1.0f);  // Top-right
  pos[baseIdx + 3] = make_float4(x0, 0.0f, z1, 1.0f);  // Top-left
}
	...
	switch (iMsg)
	{
		...
		*/
		...
		case WM_KEYDOWN:
			...
			case 0x57: // W key - move forward (decrease Z)
				cameraZ -= cameraSpeed;
				break;

			case 0x53: // S key - move backward (increase Z)
				cameraZ += cameraSpeed;
				break;

			case 0x41: // A key - move left (decrease X)
				cameraX -= cameraSpeed;
				break;

			case 0x44: // D key - move right (increase X)
				cameraX += cameraSpeed;
				break;
			}
			break;
...
VkResult initialize(void)
{
	...
	*/
	...
	// For tessellation: PATCH_GRID_SIZE x PATCH_GRID_SIZE patches, 4 vertices per patch
	// Total vertices = PATCH_GRID_SIZE * PATCH_GRID_SIZE * 4
...
VkResult UpdateUniformBuffer(void)
{
	...
	// Camera positioned to view terrain as ground plane (like bottom face of a cube)
	// Y is up, terrain extends in x-z plane at y=0
	// Camera position controlled by WASD keys (cameraX, cameraZ are global variables)
	float camHeight = 3.0f;   // Height above ground

	myUniformData.viewMatrix = glm::lookAt(
		glm::vec3(cameraX, camHeight, cameraZ),   // Camera position: controlled by WASD
		glm::vec3(cameraX, 0.0f, cameraZ - 6.0f), // Look at point ahead of camera
...
VkResult display(void)
{
	...
	*/
	...
	if(bMesh1024_chosen == TRUE)
	{
		//Run CUDA kernel for flat terrain patches with dynamic tessellation
		dim3 block(8, 8, 1);
	...
	//Common to all above mesh if blocks :- synchronize
...
{
	// Code
	// Rotation animation stopped - camera now has fixed position
}
...
VkResult CreateVulKanDevice(void)
{
	...
	*/
	...
	/*
	Enable required device features for tessellation and wireframe rendering
	*/
	VkPhysicalDeviceFeatures enabledFeatures;
	memset(&enabledFeatures, 0, sizeof(VkPhysicalDeviceFeatures));
	enabledFeatures.tessellationShader = VK_TRUE;  // Enable tessellation shaders
	enabledFeatures.fillModeNonSolid = VK_TRUE;    // Enable wireframe (VK_POLYGON_MODE_LINE)
	...
	*/
	...
	vkDeviceCreateInfo.pEnabledFeatures = &enabledFeatures;  // Enable tessellation and wireframe features
...
VkResult CreateShaders(void)
{
	...
	*/
	...
	/*
	Code for Tessellation Control Shader
	*/
	...
	/*
	Code for Tessellation Evaluation Shader
	*/
...
VkResult CreateDescriptorSetLayout()
{
	...
	*/
	...
	vkDescriptorSetLayoutBinding.stageFlags = VK_SHADER_STAGE_VERTEX_BIT|VK_SHADER_STAGE_FRAGMENT_BIT|VK_SHADER_STAGE_TESSELLATION_CONTROL_BIT|VK_SHADER_STAGE_TESSELLATION_EVALUATION_BIT; //https://registry.khronos.org/vulkan/specs/latest/man/html/VkShaderStageFlagBits.html
...
VkResult CreatePipeline(void)
{
	...
	*/
	...
	vkPipelineInputAssemblyStateCreateInfo.topology = VK_PRIMITIVE_TOPOLOGY_PATCH_LIST; // Patch list for tessellation
	...
	*/
	...
	vkPipelineRasterizationStateCreateInfo.cullMode = VK_CULL_MODE_NONE; // No culling for wireframe
	...
	*/
	...
	/*
	Tessellation State - 4 vertices per patch for quad tessellation
	*/
	...
	vkPipelineTessellationStateCreateInfo.patchControlPoints = 4; // 4 vertices per patch for quads
