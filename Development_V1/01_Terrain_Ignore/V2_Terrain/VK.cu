...
#define HEIGHTMAP_WIDTH 64
#define HEIGHTMAP_HEIGHT 64

// Patch grid dimensions for tessellation (coarse grid)
#define PATCH_GRID_WIDTH 65
#define PATCH_GRID_HEIGHT 65
...
__global__ void generateHeightMap(float* __restrict__ heightMap,
    ...
    // ==========================================================================
    // VALLEY SHAPE: Create a valley running through the center
    // ==========================================================================
    // Distance from center line (valley runs along v = 0.5)
    float distFromCenter = fabsf(v - 0.5f) * 2.0f;  // 0 at center, 1 at edges

    // Valley profile: smooth curve from valley floor to mountain slopes
    ...
    // ==========================================================================
    // TERRAIN DETAIL: Fine details and erosion patterns
    // ==========================================================================
    float detail = fbm(px * 4.0f, py * 4.0f, 4, 0.4f, 2.5f) * 0.15f;

    // Erosion-like features (turbulence)
    ...
    // Valley floor (lowest elevation)
    float valleyHeight = valleyFloor - 0.3f;

    // Mountain slopes and ridges (higher elevation)
    float mountainHeight = ridges * 0.6f + baseTerrain * 0.25f + largeFeatures * 0.2f;
    mountainHeight += detail + erosion;

    // Blend between valley and mountains based on distance from center
    heightVal = lerp(valleyHeight, mountainHeight, valleyFactor);

    // Add gentle river bed in the valley center
    float riverWidth = 0.08f;
    float riverDist = fabsf(v - 0.5f);
    if (riverDist < riverWidth) {
        float riverFactor = 1.0f - (riverDist / riverWidth);
        riverFactor = riverFactor * riverFactor;  // Smooth edges
    ...
    // ==========================================================================
    // TERRAIN EDGES: Smooth fade at boundaries
    // ==========================================================================
    // Fade terrain at edges to prevent harsh cutoffs
    ...
    // ==========================================================================
    // Scale to appropriate height range for rendering
    // Valley floor at around -0.2, mountain peaks at around 0.5
...
// ============================================================================
#define PATCH_GRID_SIZE 64  // 64x64 grid points = 63x63 quads = 7938 triangles
#define PATCH_TRIANGLE_COUNT ((PATCH_GRID_SIZE - 1) * (PATCH_GRID_SIZE - 1) * 2)
#define PATCH_VERTEX_COUNT (PATCH_TRIANGLE_COUNT * 3)  // 23814 vertices
...
VkResult UpdateUniformBuffer(void)
{
	...
	// Set up view matrix - camera positioned to see terrain in front
	// Camera at position (0, 1.5, 3) looking at origin (center of terrain)
	glm::vec3 cameraPos = glm::vec3(0.0f, 1.5f, 3.0f);    // Camera position
	...
	// The terrain spans from -1 to 1 in X and Z, so we set up ortho to capture it
	float aspectRatio = (float)winWidth / (float)winHeight;
	float orthoSize = 2.5f; // Controls how much of the scene is visible
...
VkResult CreateTessUniformBuffer(void)
{
	...
	// Initialize tessellation parameters with default values
	struct TessellationParams tessParams;
	tessParams.cameraPos = glm::vec4(0.0f, 1.5f, 3.0f, 1.0f);  // Camera position matching orthographic view setup
	tessParams.minTessLevel = 1.0f;   // Minimum tessellation for far objects
	tessParams.maxTessLevel = 32.0f;  // Maximum tessellation for close objects
	tessParams.minDistance = 0.5f;    // Distance for max tessellation
	tessParams.maxDistance = 5.0f;    // Distance for min tessellation
