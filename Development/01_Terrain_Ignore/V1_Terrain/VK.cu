...
// ============================================================================
// Generates a height map texture that will be sampled by the TES shader
// Output is a single float per pixel representing height
// ============================================================================
__global__ void generateHeightMap(float* __restrict__ heightMap,
    ...
    // Frequency for the wave pattern
    const float freq = 4.0f * 3.14159265358979f;

    // Generate procedural height using sine wave
    // Same formula as the original sinewave kernel for consistency
    float heightVal = sinf(freq * u + animTime) * cosf(freq * v + animTime) * 0.5f;

    // Add multi-octave noise for more interesting terrain
    // Octave 1: Low frequency, high amplitude (base terrain)
    heightVal += sinf(freq * 0.5f * u + animTime * 0.7f) *
                 cosf(freq * 0.5f * v + animTime * 0.7f) * 0.25f;

    // Octave 2: Higher frequency, lower amplitude (detail)
    heightVal += sinf(freq * 2.0f * u + animTime * 1.3f) *
                 cosf(freq * 2.0f * v + animTime * 1.3f) * 0.125f;
...
// ============================================================================
// Updates the height map with animated terrain for TES to sample
// ============================================================================
__global__ void updateHeightMapAnimated(float* __restrict__ heightMap,
    ...
    // Same frequency as sinewaveWithStencil for consistency
    const float freq = 4.0f * 3.14159265358979f;

    // Generate animated height using sine wave
    float heightVal = __sinf(freq * u + animTime) * __cosf(freq * v + animTime) * 0.5f;
