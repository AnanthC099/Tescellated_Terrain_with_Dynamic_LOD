...
__device__ float calculateTerrainHeight(float2 worldPos, float3 cameraPos,
    ...
    // Domain warping for organic, non-uniform terrain shapes
    ...
    height += undulations * 0.22f;

    // Subtle ridge-like features running in varied directions
    float2 ridgeCoord = make_float2(worldPos.x * 0.012f + worldPos.y * 0.004f,
                                     worldPos.y * 0.011f - worldPos.x * 0.003f);
    float ridgeNoise = noise2DSeeded(ridgeCoord, 151.0f, gradTable, permTable);
    float ridge = fabsf(ridgeNoise) * 0.7f + 0.3f;  // Soft ridge falloff
    height += (1.0f - ridge) * 0.08f;

    // Natural drainage channels / shallow gullies
    ...
    channel = channel * channel * channel;  // Sharpen channel shape
    height -= channel * 0.06f;

    // Medium-scale terrain variation for natural undulation
    float2 medCoord = make_float2(worldPos.x * 0.018f, worldPos.y * 0.022f);
    float medVariation = fbmRotatedSeeded(medCoord, 2, 137.0f, gradTable, permTable);
    height += medVariation * 0.1f;

    // Fine-scale grass clump variation
    float2 grassCoord = make_float2(worldPos.x * 0.035f, worldPos.y * 0.04f);
    float grassVariation = noise2DSeeded(grassCoord, 149.0f, gradTable, permTable);
    height += grassVariation * 0.04f;

    // Micro-scale soil irregularities - small bumps and depressions
    float2 microCoord = make_float2(worldPos.x * 0.08f, worldPos.y * 0.075f);
    float microBumps = noise2DSeeded(microCoord, 173.0f, gradTable, permTable);
    height += microBumps * 0.018f;

    // Very fine detail - ground texture variation
    float2 fineCoord = make_float2(worldPos.x * 0.15f, worldPos.y * 0.16f);
    float fineDetail = noise2DSeeded(fineCoord, 191.0f, gradTable, permTable);
    height += fineDetail * 0.008f;

    // Scattered small mounds (animal burrows, roots, etc.)
    float2 moundCoord = make_float2(worldPos.x * 0.025f, worldPos.y * 0.028f);
    float moundNoise = noise2DSeeded(moundCoord, 199.0f, gradTable, permTable);
    float mound = fmaxf(0.0f, moundNoise - 0.6f) * 2.5f;  // Only peaks become mounds
    height += mound * mound * 0.03f;
    ...
    // Simulates bunches of grass growing together in small clumps
    float2 tuftCoord = make_float2(worldPos.x * 0.5f, worldPos.y * 0.55f);
    float tuftNoise = noise2DSeeded(tuftCoord, 211.0f, gradTable, permTable);
    float tuft = fmaxf(0.0f, tuftNoise) * tuftNoise;  // Squared for sharper peaks
    height += tuft * 0.035f;

    // Secondary grass clumps at different scale
    float2 clumpCoord = make_float2(worldPos.x * 0.8f, worldPos.y * 0.75f);
    float clumpNoise = noise2DSeeded(clumpCoord, 223.0f, gradTable, permTable);
    float clump = fmaxf(0.0f, clumpNoise - 0.2f);
    height += clump * clump * 0.025f;

    // Fine grass blade variation - very high frequency bumps
    float2 bladeCoord = make_float2(worldPos.x * 1.5f, worldPos.y * 1.4f);
    float bladeNoise = noise2DSeeded(bladeCoord, 227.0f, gradTable, permTable);
    height += bladeNoise * 0.012f;

    // Ultra-fine surface granularity - soil and root texture
    float2 grainCoord = make_float2(worldPos.x * 2.5f, worldPos.y * 2.3f);
    float grainNoise = noise2DSeeded(grainCoord, 229.0f, gradTable, permTable);
    height += grainNoise * 0.006f;

    // Patchy grass density - some areas thicker than others
    float2 patchCoord = make_float2(worldPos.x * 0.12f, worldPos.y * 0.14f);
    ...
    float2 thickCoord = make_float2(worldPos.x * 0.6f, worldPos.y * 0.65f);
    float thickNoise = noise2DSeeded(thickCoord, 239.0f, gradTable, permTable);
    height += thickNoise * 0.02f * patchMask;

    // Small depressions between grass clumps
    float2 gapCoord = make_float2(worldPos.x * 0.4f, worldPos.y * 0.38f);
    float gapNoise = noise2DSeeded(gapCoord, 241.0f, gradTable, permTable);
    float gap = fminf(0.0f, gapNoise + 0.3f);  // Only negative values (depressions)
    height += gap * 0.018f;
