...
VkResult UpdateUniformBuffer(void)
{
	...
	// Camera position for viewing the terrain - orbit around based on animation time
	float camRadius = 5.0f;
	float camHeight = 3.0f;
	float camX = camRadius * sinf(animationTime * 0.2f);
	float camZ = camRadius * cosf(animationTime * 0.2f);

	myUniformData.viewMatrix = glm::lookAt(
		glm::vec3(camX, camHeight, camZ),  // Camera position (orbiting)
