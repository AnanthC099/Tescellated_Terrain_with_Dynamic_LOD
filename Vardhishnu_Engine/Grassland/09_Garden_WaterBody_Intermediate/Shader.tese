...
layout(location = 2) out float outWaterMask;  // Water body indicator for fragment shader
...
float getBedEdgeMask(vec2 p) {
    ...
    // (dFdx/dFdy are only available in fragment shaders)
    float eps = 2.0;
...
// Terrace mask - for stepped landscaping
float getTerraceMask(vec2 worldPos) {
    float terraceHeight = worldPos.y; // Will be computed based on preliminary height
    return 0.0; // Placeholder - computed after initial height
}
...
// ============================================================================
// WATER BODY MASK - Identifies regions that should be water
// ============================================================================

// Water level constant (normalized, will be multiplied by heightScale)
const float waterLevel = -0.08;  // Slightly below average terrain level

// Water body mask - creates natural pond/lake shapes in low-lying areas
float getWaterBodyMask(vec2 worldPos) {
    // Use domain-warped coordinates for organic shapes
    vec2 warpedPos = domainWarpStrong(worldPos * 0.003, 0.5);

    // Primary water body regions - large ponds/lakes
    float primaryWater = fbmBillowy(warpedPos + vec2(400.0, 250.0), 4);
    float primary = smoothstep(0.35, 0.55, primaryWater);

    // Secondary smaller pools using different offset
    float secondaryWater = fbmRotated(warpedPos * 1.5 + vec2(150.0, 350.0), 3);
    float secondary = smoothstep(0.45, 0.6, secondaryWater) * 0.6;

    // Combine with basin mask to ensure water is in low areas
    float basinMask = getBasinMask(worldPos);

    // Water bodies form where basin mask is high AND water noise is favorable
    float waterMask = max(primary, secondary) * basinMask;

    // Add some isolated small ponds
    float isolatedPonds = cellularNoise(warpedPos * 2.0 + vec2(80.0, 120.0));
    isolatedPonds = smoothstep(0.02, 0.08, isolatedPonds) * 0.4;

    return clamp(waterMask + isolatedPonds * basinMask, 0.0, 1.0);
}
...
// ============================================================================
// WATER BODY APPLICATION - Flatten terrain to water level in water regions
// ============================================================================

float applyWaterBodies(float height, vec2 worldPos, out float waterMask) {
    waterMask = getWaterBodyMask(worldPos);

    // Only apply water where terrain is below water level AND in water region
    if (waterMask > 0.01 && height < waterLevel) {
        // Smooth transition at water edges
        float waterBlend = smoothstep(0.0, 0.3, waterMask);

        // In water regions below water level, flatten to water surface
        // Add subtle water surface variation for visual interest
        vec2 waterCoord = worldPos * 0.02;
        float waterRipple = simplexNoise(waterCoord + vec2(100.0, 200.0)) * 0.005;

        float waterSurfaceHeight = waterLevel + waterRipple;

        // Blend between terrain and water surface based on water mask
        height = mix(height, waterSurfaceHeight, waterBlend);
    }
...
float calculateTerrainHeight(vec2 worldPos, vec3 cameraPos, out float waterMask) {
    // Get control masks
    ...
    // Apply water bodies - flatten terrain to water level in water regions
    height = applyWaterBodies(height, worldPos, waterMask);

    return height;
}

// Overload for normal calculation (doesn't need water mask output)
float calculateTerrainHeight(vec2 worldPos, vec3 cameraPos) {
    float unusedWaterMask;
    return calculateTerrainHeight(worldPos, cameraPos, unusedWaterMask);
}
...
void main() {
    ...
    // Calculate terrain height using layered system (with water mask output)
    float waterMask;
    float height = calculateTerrainHeight(worldPos, cameraPos, waterMask);
    ...
    float eps = 1.0;
    float hRight = calculateTerrainHeight(worldPos + vec2(eps, 0.0), cameraPos);
    float hLeft = calculateTerrainHeight(worldPos - vec2(eps, 0.0), cameraPos);
    float hUp = calculateTerrainHeight(worldPos + vec2(0.0, eps), cameraPos);
    float hDown = calculateTerrainHeight(worldPos - vec2(0.0, eps), cameraPos);

    vec3 normal = normalize(vec3(
        (hLeft - hRight) * heightScale,
        2.0 * eps,
        (hDown - hUp) * heightScale
    ));

    outWorldPos = position.xyz;
    outNormal = normal;
    outWaterMask = waterMask;  // Pass water mask to fragment shader
