...
#include <stdlib.h>
#include <windows.h>
#include <windowsx.h>  // For GET_X_LPARAM, GET_Y_LPARAM, GET_WHEEL_DELTA_WPARAM
#include <math.h>
#include <stdarg.h>
#include <vector>
...
*/
const char* enabledDeviceExtensionNames_array[10]; // Increased for ray tracing extensions

// ============================================================================
// VULKAN RAY TRACING API SUPPORT
// ============================================================================
BOOL bRayTracingSupported = FALSE;

// Ray tracing extension function pointers (loaded dynamically)
PFN_vkGetBufferDeviceAddressKHR fpGetBufferDeviceAddressKHR = NULL;
PFN_vkCreateAccelerationStructureKHR fpCreateAccelerationStructureKHR = NULL;
PFN_vkDestroyAccelerationStructureKHR fpDestroyAccelerationStructureKHR = NULL;
PFN_vkGetAccelerationStructureBuildSizesKHR fpGetAccelerationStructureBuildSizesKHR = NULL;
PFN_vkCmdBuildAccelerationStructuresKHR fpCmdBuildAccelerationStructuresKHR = NULL;
PFN_vkGetAccelerationStructureDeviceAddressKHR fpGetAccelerationStructureDeviceAddressKHR = NULL;

// Bottom Level Acceleration Structure (BLAS) - for ground plane geometry
VkAccelerationStructureKHR vkAccelerationStructure_BLAS = VK_NULL_HANDLE;
VkBuffer vkBuffer_BLAS = VK_NULL_HANDLE;
VkDeviceMemory vkDeviceMemory_BLAS = VK_NULL_HANDLE;
VkDeviceAddress vkDeviceAddress_BLAS = 0;

// Top Level Acceleration Structure (TLAS) - scene structure
VkAccelerationStructureKHR vkAccelerationStructure_TLAS = VK_NULL_HANDLE;
VkBuffer vkBuffer_TLAS = VK_NULL_HANDLE;
VkDeviceMemory vkDeviceMemory_TLAS = VK_NULL_HANDLE;
VkDeviceAddress vkDeviceAddress_TLAS = 0;

// Instance buffer for TLAS
VkBuffer vkBuffer_TLASInstances = VK_NULL_HANDLE;
VkDeviceMemory vkDeviceMemory_TLASInstances = VK_NULL_HANDLE;

// Scratch buffer for acceleration structure building
VkBuffer vkBuffer_ASScratch = VK_NULL_HANDLE;
VkDeviceMemory vkDeviceMemory_ASScratch = VK_NULL_HANDLE;

// Ground plane vertex/index buffers for BLAS
VkBuffer vkBuffer_GroundVertices = VK_NULL_HANDLE;
VkDeviceMemory vkDeviceMemory_GroundVertices = VK_NULL_HANDLE;
VkBuffer vkBuffer_GroundIndices = VK_NULL_HANDLE;
VkDeviceMemory vkDeviceMemory_GroundIndices = VK_NULL_HANDLE;

// Ray tracing properties
VkPhysicalDeviceRayTracingPipelinePropertiesKHR rtPipelineProperties = {};
VkPhysicalDeviceAccelerationStructurePropertiesKHR rtAccelStructProperties = {};
...
// This is independent of swapchain image count and controls CPU/GPU parallelism
#define MAX_FRAMES_IN_FLIGHT 2

//https://registry.khronos.org/vulkan/specs/latest/man/html/VkSemaphore.html
// Per-frame semaphores for proper synchronization (indexed by currentFrame)
VkSemaphore vkSemaphore_ImageAvailable_array[MAX_FRAMES_IN_FLIGHT];
VkSemaphore vkSemaphore_RenderComplete_array[MAX_FRAMES_IN_FLIGHT];

//https://registry.khronos.org/vulkan/specs/latest/man/html/VkFence.html
// Per-frame fences (indexed by currentFrame)
VkFence vkFence_InFlight_array[MAX_FRAMES_IN_FLIGHT];

// Track which fence is currently using each swapchain image (indexed by image index)
// This handles the case where we acquire an image still in use by a previous frame
VkFence *vkFence_ImagesInFlight_array = NULL;

uint32_t currentFrame = 0; // Tracks which frame we're rendering (cycles 0 to MAX_FRAMES_IN_FLIGHT-1)
...
VkImage vkImage_texture = VK_NULL_HANDLE; //https://registry.khronos.org/vulkan/specs/latest/man/html/VkImage.html
VkDeviceMemory vkDeviceMemory_texture = VK_NULL_HANDLE; //https://registry.khronos.org/vulkan/specs/latest/man/html/VkDeviceMemory.html
VkImageView vkImageView_texture = VK_NULL_HANDLE; //https://registry.khronos.org/vulkan/specs/latest/man/html/VkImageView.html
VkSampler vkSampler_texture = VK_NULL_HANDLE; //https://registry.khronos.org/vulkan/specs/latest/man/html/VkSampler.html

// Compute shader related global variables for procedural clouds
VkShaderModule vkShaderModule_compute = VK_NULL_HANDLE;
...
VkImage vkImage_computeOutput = VK_NULL_HANDLE;
VkDeviceMemory vkDeviceMemory_computeOutput = VK_NULL_HANDLE;
VkImageView vkImageView_computeOutput = VK_NULL_HANDLE;

// ============================================================================
// TEMPORAL REPROJECTION - History buffers for accumulating samples over time
// Uses ping-pong approach: read from history, blend with current, write to output
// ============================================================================
VkImage vkImage_historyBuffer = VK_NULL_HANDLE;
VkDeviceMemory vkDeviceMemory_historyBuffer = VK_NULL_HANDLE;
VkImageView vkImageView_historyBuffer = VK_NULL_HANDLE;
VkSampler vkSampler_history = VK_NULL_HANDLE;

// Frame index for temporal accumulation
uint32_t temporalFrameIndex = 0;

// Cloud params uniform buffer for compute shader
struct CloudParamsData {
	glm::vec4 cameraPos;         // xyz = position, w = time
	glm::vec4 sunDirection;      // xyz = normalized sun direction, w = unused
	glm::vec4 screenSize;        // xy = width/height, zw = unused
	glm::vec4 cameraForward;     // xyz = forward direction, w = unused
	glm::vec4 cameraRight;       // xyz = right direction, w = unused
	glm::vec4 cameraUp;          // xyz = up direction, w = fov (radians)
	// Temporal reprojection data
	glm::vec4 prevCameraPos;     // xyz = previous camera position, w = frame index
	glm::vec4 prevCameraForward; // xyz = previous forward direction, w = blend factor
	glm::vec4 prevCameraRight;   // xyz = previous right direction, w = unused
	glm::vec4 prevCameraUp;      // xyz = previous up direction, w = previous fov
};
...
CloudParamsBuffer cloudParamsBuffer;
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	{
		// Skip logging "Program started successfully" - expected startup message
	}
	...
	switch (iMsg)
	{
		...
		case WM_SIZE:
			...
			else
			{
				...
				if (vkResult != VK_SUCCESS)
				{
					// Skip logging if init not complete (expected during startup)
...
VkResult initialize(void)
{
	...
	*/
	...
	// Compute shader function declarations for procedural clouds
	VkResult CreateComputeStorageImage(void);
	VkResult CreateTemporalHistoryBuffer(void);
	VkResult CreateHistorySampler(void);
	VkResult CreateCloudParamsUniformBuffer(void);
	VkResult UpdateCloudParamsBuffer(void);
	VkResult CreateComputeShader(void);
	...
	VkResult CreateComputeDescriptorPool(void);
	VkResult CreateComputeDescriptorSet(void);
	VkResult CreateComputeTextureSampler(void);
	void CopyOutputToHistory(VkCommandBuffer cmdBuffer);
	...
	*/
	...
	// ============ COMPUTE SHADER SETUP FOR PROCEDURAL CLOUDS ============
	// Create storage image for compute shader output
	vkResult = CreateComputeStorageImage();
	if (vkResult != VK_SUCCESS)
	{
		fprintf(gFILE, "initialize(): CreateComputeStorageImage() function failed with error code %d\n", vkResult);
		return vkResult;
	}
	else
	{
		fprintf(gFILE, "initialize(): CreateComputeStorageImage() succedded\n");
	}

	// Create temporal history buffer for reprojection
	vkResult = CreateTemporalHistoryBuffer();
	if (vkResult != VK_SUCCESS)
	{
		fprintf(gFILE, "initialize(): CreateTemporalHistoryBuffer() function failed with error code %d\n", vkResult);
		return vkResult;
	}
	else
	{
		fprintf(gFILE, "initialize(): CreateTemporalHistoryBuffer() succedded\n");
	}

	// Create sampler for history buffer
	vkResult = CreateHistorySampler();
	if (vkResult != VK_SUCCESS)
	{
		fprintf(gFILE, "initialize(): CreateHistorySampler() function failed with error code %d\n", vkResult);
		return vkResult;
	}
	else
	{
		fprintf(gFILE, "initialize(): CreateHistorySampler() succedded\n");
	}

	// Create cloud params uniform buffer
	vkResult = CreateCloudParamsUniformBuffer();
	if (vkResult != VK_SUCCESS)
	{
		fprintf(gFILE, "initialize(): CreateCloudParamsUniformBuffer() function failed with error code %d\n", vkResult);
		return vkResult;
	}
	else
	{
		fprintf(gFILE, "initialize(): CreateCloudParamsUniformBuffer() succedded\n");
	}

	// Create compute shader
	vkResult = CreateComputeShader();
	if (vkResult != VK_SUCCESS)
	{
		fprintf(gFILE, "initialize(): CreateComputeShader() function failed with error code %d\n", vkResult);
		return vkResult;
	}
	else
	{
		fprintf(gFILE, "initialize(): CreateComputeShader() succedded\n");
	}
	...
	if (vkResult != VK_SUCCESS)
	{
		fprintf(gFILE, "initialize(): CreateComputeDescriptorSetLayout() function failed with error code %d\n", vkResult);
		return vkResult;
	}
	else
	{
		fprintf(gFILE, "initialize(): CreateComputeDescriptorSetLayout() succedded\n");
	}
	...
	if (vkResult != VK_SUCCESS)
	{
		fprintf(gFILE, "initialize(): CreateComputePipelineLayout() function failed with error code %d\n", vkResult);
		return vkResult;
	}
	else
	{
		fprintf(gFILE, "initialize(): CreateComputePipelineLayout() succedded\n");
	}
	...
	if (vkResult != VK_SUCCESS)
	{
		fprintf(gFILE, "initialize(): CreateComputePipeline() function failed with error code %d\n", vkResult);
		return vkResult;
	}
	else
	{
		fprintf(gFILE, "initialize(): CreateComputePipeline() succedded\n");
	}
	...
	if (vkResult != VK_SUCCESS)
	{
		fprintf(gFILE, "initialize(): CreateComputeDescriptorPool() function failed with error code %d\n", vkResult);
		return vkResult;
	}
	else
	{
		fprintf(gFILE, "initialize(): CreateComputeDescriptorPool() succedded\n");
	}
	...
	if (vkResult != VK_SUCCESS)
	{
		fprintf(gFILE, "initialize(): CreateComputeDescriptorSet() function failed with error code %d\n", vkResult);
		return vkResult;
	}
	else
	{
		fprintf(gFILE, "initialize(): CreateComputeDescriptorSet() succedded\n");
	}

	// Create sampler for compute output texture
	vkResult = CreateComputeTextureSampler();
	if (vkResult != VK_SUCCESS)
	{
		fprintf(gFILE, "initialize(): CreateComputeTextureSampler() function failed with error code %d\n", vkResult);
		return vkResult;
	}
	else
	{
		fprintf(gFILE, "initialize(): CreateComputeTextureSampler() succedded\n");
	}
	// ============ END OF COMPUTE SHADER SETUP ============

	// ============ RAY TRACING INITIALIZATION ============
	// Create acceleration structures if ray tracing is supported
	VkResult CreateRayTracingAccelerationStructures(void);
	VkResult UpdateComputeDescriptorSetWithAccelerationStructure(void);

	if (!bRayTracingSupported)
	{
		fprintf(gFILE, "initialize(): Ray tracing required but not supported\n");
		return VK_ERROR_INITIALIZATION_FAILED;
	}

	vkResult = CreateRayTracingAccelerationStructures();
	if (vkResult != VK_SUCCESS)
	{
		fprintf(gFILE, "initialize(): CreateRayTracingAccelerationStructures() function failed with error code %d\n", vkResult);
		return vkResult;
	}
	fprintf(gFILE, "initialize(): CreateRayTracingAccelerationStructures() succedded\n");

	// Update compute descriptor set with acceleration structure
	vkResult = UpdateComputeDescriptorSetWithAccelerationStructure();
	if (vkResult != VK_SUCCESS)
	{
		fprintf(gFILE, "initialize(): UpdateComputeDescriptorSetWithAccelerationStructure() function failed\n");
		return vkResult;
	}
	fprintf(gFILE, "initialize(): UpdateComputeDescriptorSetWithAccelerationStructure() succedded\n");
	// ============ END OF RAY TRACING INITIALIZATION ============
...
VkResult resize(int width, int height)
{
	...
	VkResult CreateComputeStorageImage(void);
	VkResult CreateTemporalHistoryBuffer(void);
	VkResult UpdateDescriptorSetsForNewStorageImage(void);
	
	//Variable declarations
	VkResult vkResult = VK_SUCCESS;
	
	// Code
	// Validate dimensions to prevent zero-sized swapchain (can happen during fullscreen transitions)
	if(width <= 0)
	{
		width = 1;
	}
	...
	{
		// Skip logging - expected during startup before init completes
	...
	*/
	...
	// Reset frame counter and images-in-flight tracking after waiting for device idle
	currentFrame = 0;
	for(uint32_t i = 0; i < swapchainImageCount; i++)
	{
		vkFence_ImagesInFlight_array[i] = VK_NULL_HANDLE;
	}
	...
	// Destroy compute storage image resources for resize
	if(vkImageView_computeOutput)
	{
		vkDestroyImageView(vkDevice, vkImageView_computeOutput, NULL);
		vkImageView_computeOutput = VK_NULL_HANDLE;
		fprintf(gFILE, "resize(): vkImageView_computeOutput is freed\n");
	}

	if(vkDeviceMemory_computeOutput)
	{
		vkFreeMemory(vkDevice, vkDeviceMemory_computeOutput, NULL);
		vkDeviceMemory_computeOutput = VK_NULL_HANDLE;
		fprintf(gFILE, "resize(): vkDeviceMemory_computeOutput is freed\n");
	}

	if(vkImage_computeOutput)
	{
		vkDestroyImage(vkDevice, vkImage_computeOutput, NULL);
		vkImage_computeOutput = VK_NULL_HANDLE;
		fprintf(gFILE, "resize(): vkImage_computeOutput is freed\n");
	}

	// Destroy temporal history buffer resources for resize
	if(vkImageView_historyBuffer)
	{
		vkDestroyImageView(vkDevice, vkImageView_historyBuffer, NULL);
		vkImageView_historyBuffer = VK_NULL_HANDLE;
		fprintf(gFILE, "resize(): vkImageView_historyBuffer is freed\n");
	}

	if(vkDeviceMemory_historyBuffer)
	{
		vkFreeMemory(vkDevice, vkDeviceMemory_historyBuffer, NULL);
		vkDeviceMemory_historyBuffer = VK_NULL_HANDLE;
		fprintf(gFILE, "resize(): vkDeviceMemory_historyBuffer is freed\n");
	}

	if(vkImage_historyBuffer)
	{
		vkDestroyImage(vkDevice, vkImage_historyBuffer, NULL);
		vkImage_historyBuffer = VK_NULL_HANDLE;
		fprintf(gFILE, "resize(): vkImage_historyBuffer is freed\n");
	}

	// Reset temporal frame index on resize
	...
	*/
	...
	// Recreate compute storage image with new swapchain dimensions
	vkResult = CreateComputeStorageImage();
	if (vkResult != VK_SUCCESS)
	{
		fprintf(gFILE, "resize(): CreateComputeStorageImage() function failed with error code %d\n", vkResult);
		return vkResult;
	}

	// Recreate temporal history buffer for new dimensions
	vkResult = CreateTemporalHistoryBuffer();
	if (vkResult != VK_SUCCESS)
	{
		fprintf(gFILE, "resize(): CreateTemporalHistoryBuffer() function failed with error code %d\n", vkResult);
		return vkResult;
	}

	// Update descriptor sets to reference the new storage image
	vkResult = UpdateDescriptorSetsForNewStorageImage();
	if (vkResult != VK_SUCCESS)
	{
		fprintf(gFILE, "resize(): UpdateDescriptorSetsForNewStorageImage() function failed with error code %d\n", vkResult);
		return vkResult;
	}
...
VkResult UpdateUniformBuffer(void)
{
	...
	//Update matrices - Identity matrices for fullscreen quad (no rotation, no transformation)
	//Vertices are already in NDC coordinates (-1 to 1), so identity matrices render fullscreen
...
VkResult display(void)
{
	...
	/*
	Step 1: Wait for the in-flight fence of currentFrame to ensure the previous
	frame using these sync objects has completed before we reuse them.
	*/
	//https://registry.khronos.org/vulkan/specs/latest/man/html/vkWaitForFences.html
	vkResult = vkWaitForFences(vkDevice, 1, &vkFence_InFlight_array[currentFrame], VK_TRUE, UINT64_MAX);
	if(vkResult != VK_SUCCESS)
	{
	...
	/*
	Step 2: Acquire index of next swapChainImage
	*/
	//https://registry.khronos.org/vulkan/specs/latest/man/html/vkAcquireNextImageKHR.html
	// Use per-frame semaphore to avoid pending operations on the same semaphore
	vkResult = vkAcquireNextImageKHR(vkDevice, vkSwapchainKHR, UINT64_MAX, vkSemaphore_ImageAvailable_array[currentFrame], VK_NULL_HANDLE, &currentImageIndex);
	if(vkResult != VK_SUCCESS)
	{
		if((vkResult == VK_ERROR_OUT_OF_DATE_KHR) || (vkResult == VK_SUBOPTIMAL_KHR))
		{
			resize(winWidth, winHeight);
			return VK_SUCCESS; // Swapchain was recreated, skip this frame
		}
		else
		{
	...
	/*
	Step 3: Check if a previous frame is still using this image (i.e., there is a fence to wait on)
	This handles the case where MAX_FRAMES_IN_FLIGHT > swapchainImageCount, or when
	images are acquired out of order.
	*/
	if(vkFence_ImagesInFlight_array[currentImageIndex] != VK_NULL_HANDLE)
	{
		vkResult = vkWaitForFences(vkDevice, 1, &vkFence_ImagesInFlight_array[currentImageIndex], VK_TRUE, UINT64_MAX);
		if(vkResult != VK_SUCCESS)
		{
			fprintf(gFILE, "display(): vkWaitForFences() for image in flight failed\n");
			return vkResult;
		}
	}

	// Mark this image as now being in use by this frame's fence
	vkFence_ImagesInFlight_array[currentImageIndex] = vkFence_InFlight_array[currentFrame];

	/*
	Step 4: Reset the fence for this frame (only after we know it's been signaled)
	*/
	//https://registry.khronos.org/vulkan/specs/latest/man/html/vkResetFences.html
	vkResult = vkResetFences(vkDevice, 1, &vkFence_InFlight_array[currentFrame]);
	if(vkResult != VK_SUCCESS)
	{
	...
	vkSubmitInfo.pWaitSemaphores = &vkSemaphore_ImageAvailable_array[currentFrame];
	vkSubmitInfo.commandBufferCount = 1;
	vkSubmitInfo.pCommandBuffers = &vkCommandBuffer_array[currentImageIndex];
	vkSubmitInfo.signalSemaphoreCount = 1;
	vkSubmitInfo.pSignalSemaphores = &vkSemaphore_RenderComplete_array[currentFrame];

	//Now submit above work to the queque
	vkResult = vkQueueSubmit(vkQueue, 1, &vkSubmitInfo, vkFence_InFlight_array[currentFrame]); //https://registry.khronos.org/vulkan/specs/latest/man/html/vkQueueSubmit.html
	if(vkResult != VK_SUCCESS)
	{
	...
	vkPresentInfoKHR.pWaitSemaphores = &vkSemaphore_RenderComplete_array[currentFrame];
	...
	// Advance to next frame (cycles through 0 to MAX_FRAMES_IN_FLIGHT-1 for proper frame synchronization)
	...
	// Update cloud parameters for animation
	vkResult = UpdateCloudParamsBuffer();
	if(vkResult != VK_SUCCESS)
	{
		fprintf(gFILE, "display(): UpdateCloudParamsBuffer() function failed with error code %d\n", vkResult);
		return vkResult;
	}
...
void uninitialize(void)
{
		...
		if(vkDevice)
		{
			...
			/*
			18_7. Destroy per-frame fences (MAX_FRAMES_IN_FLIGHT) and the images-in-flight tracking array.
			*/
			//Destroying per-frame fences
			for(int i = 0; i < MAX_FRAMES_IN_FLIGHT; i++)
			{
				if(vkFence_InFlight_array[i] != VK_NULL_HANDLE)
				{
					vkDestroyFence(vkDevice, vkFence_InFlight_array[i], NULL);
					vkFence_InFlight_array[i] = VK_NULL_HANDLE;
					fprintf(gFILE, "uninitialize(): vkFence_InFlight_array[%d] is freed\n", i);
				}
			}

			// Free images-in-flight tracking array (note: these are just references, not owned fences)
			if(vkFence_ImagesInFlight_array)
			{
				free(vkFence_ImagesInFlight_array);
				vkFence_ImagesInFlight_array = NULL;
				fprintf(gFILE, "uninitialize(): vkFence_ImagesInFlight_array is freed\n");
			}

			/*
			18_8. Destroy per-frame semaphores (MAX_FRAMES_IN_FLIGHT).
			*/
			//https://registry.khronos.org/vulkan/specs/latest/man/html/vkDestroySemaphore.html
			for(int i = 0; i < MAX_FRAMES_IN_FLIGHT; i++)
			{
				if(vkSemaphore_RenderComplete_array[i] != VK_NULL_HANDLE)
				{
					vkDestroySemaphore(vkDevice, vkSemaphore_RenderComplete_array[i], NULL);
					vkSemaphore_RenderComplete_array[i] = VK_NULL_HANDLE;
					fprintf(gFILE, "uninitialize(): vkSemaphore_RenderComplete_array[%d] is freed\n", i);
				}

				if(vkSemaphore_ImageAvailable_array[i] != VK_NULL_HANDLE)
				{
					vkDestroySemaphore(vkDevice, vkSemaphore_ImageAvailable_array[i], NULL);
					vkSemaphore_ImageAvailable_array[i] = VK_NULL_HANDLE;
					fprintf(gFILE, "uninitialize(): vkSemaphore_ImageAvailable_array[%d] is freed\n", i);
				}
			}
			...
			*/
			...
			if(vkPipeline_compute)
			{
				...
				fprintf(gFILE, "uninitialize(): vkPipeline_compute is freed\n");
			}
			...
			if(vkDescriptorPool_compute)
			{
				...
				fprintf(gFILE, "uninitialize(): vkDescriptorPool_compute is freed\n");
			}
			...
			if(vkDescriptorSetLayout_compute)
			{
				...
				fprintf(gFILE, "uninitialize(): vkDescriptorSetLayout_compute is freed\n");
			}
			...
			if(vkPipelineLayout_compute)
			{
				...
				fprintf(gFILE, "uninitialize(): vkPipelineLayout_compute is freed\n");
			}

			if(vkShaderModule_compute)
			{
				vkDestroyShaderModule(vkDevice, vkShaderModule_compute, NULL);
				vkShaderModule_compute = VK_NULL_HANDLE;
				fprintf(gFILE, "uninitialize(): vkShaderModule_compute is freed\n");
			}

			// Cloud params uniform buffer
			if(cloudParamsBuffer.vkBuffer)
			{
				vkDestroyBuffer(vkDevice, cloudParamsBuffer.vkBuffer, NULL);
				cloudParamsBuffer.vkBuffer = VK_NULL_HANDLE;
				fprintf(gFILE, "uninitialize(): cloudParamsBuffer.vkBuffer is freed\n");
			}

			if(cloudParamsBuffer.vkDeviceMemory)
			{
				vkFreeMemory(vkDevice, cloudParamsBuffer.vkDeviceMemory, NULL);
				cloudParamsBuffer.vkDeviceMemory = VK_NULL_HANDLE;
				fprintf(gFILE, "uninitialize(): cloudParamsBuffer.vkDeviceMemory is freed\n");
			}

			// Compute output storage image
			if(vkImageView_computeOutput)
			{
				vkDestroyImageView(vkDevice, vkImageView_computeOutput, NULL);
				vkImageView_computeOutput = VK_NULL_HANDLE;
				fprintf(gFILE, "uninitialize(): vkImageView_computeOutput is freed\n");
			}
			...
			if(vkDeviceMemory_computeOutput)
			{
				vkFreeMemory(vkDevice, vkDeviceMemory_computeOutput, NULL);
				vkDeviceMemory_computeOutput = VK_NULL_HANDLE;
				fprintf(gFILE, "uninitialize(): vkDeviceMemory_computeOutput is freed\n");
			}
			...
			if(vkImage_computeOutput)
			{
				vkDestroyImage(vkDevice, vkImage_computeOutput, NULL);
				vkImage_computeOutput = VK_NULL_HANDLE;
				fprintf(gFILE, "uninitialize(): vkImage_computeOutput is freed\n");
			}

			// Temporal history buffer cleanup
			if(vkImageView_historyBuffer)
			{
				vkDestroyImageView(vkDevice, vkImageView_historyBuffer, NULL);
				vkImageView_historyBuffer = VK_NULL_HANDLE;
				fprintf(gFILE, "uninitialize(): vkImageView_historyBuffer is freed\n");
			}
			...
			if(vkDeviceMemory_historyBuffer)
			{
				vkFreeMemory(vkDevice, vkDeviceMemory_historyBuffer, NULL);
				vkDeviceMemory_historyBuffer = VK_NULL_HANDLE;
				fprintf(gFILE, "uninitialize(): vkDeviceMemory_historyBuffer is freed\n");
			}
			...
			if(vkImage_historyBuffer)
			{
				vkDestroyImage(vkDevice, vkImage_historyBuffer, NULL);
				vkImage_historyBuffer = VK_NULL_HANDLE;
				fprintf(gFILE, "uninitialize(): vkImage_historyBuffer is freed\n");
			}

			// History buffer sampler
			if(vkSampler_history)
			{
				vkDestroySampler(vkDevice, vkSampler_history, NULL);
				vkSampler_history = VK_NULL_HANDLE;
				fprintf(gFILE, "uninitialize(): vkSampler_history is freed\n");
			}
			// ============ END OF COMPUTE SHADER CLEANUP ============

			// ============ RAY TRACING CLEANUP ============
			// Clean up RT resources regardless of bRayTracingSupported to handle partial init failures
			// Acceleration structures require fpDestroyAccelerationStructureKHR, but buffers/memory don't
			{
				bool hasRTResources = false;

				// Destroy TLAS (requires function pointer)
				if (vkAccelerationStructure_TLAS != VK_NULL_HANDLE)
				{
					if (fpDestroyAccelerationStructureKHR != NULL)
					{
						fpDestroyAccelerationStructureKHR(vkDevice, vkAccelerationStructure_TLAS, NULL);
						fprintf(gFILE, "uninitialize(): vkAccelerationStructure_TLAS is freed\n");
					}
					else
					{
						fprintf(gFILE, "uninitialize(): WARNING - Cannot free vkAccelerationStructure_TLAS (function pointer not loaded)\n");
					}
					vkAccelerationStructure_TLAS = VK_NULL_HANDLE;
					hasRTResources = true;
				}

				if (vkBuffer_TLAS != VK_NULL_HANDLE)
				{
					vkDestroyBuffer(vkDevice, vkBuffer_TLAS, NULL);
					vkBuffer_TLAS = VK_NULL_HANDLE;
					fprintf(gFILE, "uninitialize(): vkBuffer_TLAS is freed\n");
					hasRTResources = true;
				}

				if (vkDeviceMemory_TLAS != VK_NULL_HANDLE)
				{
					vkFreeMemory(vkDevice, vkDeviceMemory_TLAS, NULL);
					vkDeviceMemory_TLAS = VK_NULL_HANDLE;
					fprintf(gFILE, "uninitialize(): vkDeviceMemory_TLAS is freed\n");
					hasRTResources = true;
				}

				// Destroy TLAS instance buffer
				if (vkBuffer_TLASInstances != VK_NULL_HANDLE)
				{
					vkDestroyBuffer(vkDevice, vkBuffer_TLASInstances, NULL);
					vkBuffer_TLASInstances = VK_NULL_HANDLE;
					fprintf(gFILE, "uninitialize(): vkBuffer_TLASInstances is freed\n");
					hasRTResources = true;
				}

				if (vkDeviceMemory_TLASInstances != VK_NULL_HANDLE)
				{
					vkFreeMemory(vkDevice, vkDeviceMemory_TLASInstances, NULL);
					vkDeviceMemory_TLASInstances = VK_NULL_HANDLE;
					fprintf(gFILE, "uninitialize(): vkDeviceMemory_TLASInstances is freed\n");
					hasRTResources = true;
				}

				// Destroy BLAS (requires function pointer)
				if (vkAccelerationStructure_BLAS != VK_NULL_HANDLE)
				{
					if (fpDestroyAccelerationStructureKHR != NULL)
					{
						fpDestroyAccelerationStructureKHR(vkDevice, vkAccelerationStructure_BLAS, NULL);
						fprintf(gFILE, "uninitialize(): vkAccelerationStructure_BLAS is freed\n");
					}
					else
					{
						fprintf(gFILE, "uninitialize(): WARNING - Cannot free vkAccelerationStructure_BLAS (function pointer not loaded)\n");
					}
					vkAccelerationStructure_BLAS = VK_NULL_HANDLE;
					hasRTResources = true;
				}

				if (vkBuffer_BLAS != VK_NULL_HANDLE)
				{
					vkDestroyBuffer(vkDevice, vkBuffer_BLAS, NULL);
					vkBuffer_BLAS = VK_NULL_HANDLE;
					fprintf(gFILE, "uninitialize(): vkBuffer_BLAS is freed\n");
					hasRTResources = true;
				}

				if (vkDeviceMemory_BLAS != VK_NULL_HANDLE)
				{
					vkFreeMemory(vkDevice, vkDeviceMemory_BLAS, NULL);
					vkDeviceMemory_BLAS = VK_NULL_HANDLE;
					fprintf(gFILE, "uninitialize(): vkDeviceMemory_BLAS is freed\n");
					hasRTResources = true;
				}

				// Destroy scratch buffer
				if (vkBuffer_ASScratch != VK_NULL_HANDLE)
				{
					vkDestroyBuffer(vkDevice, vkBuffer_ASScratch, NULL);
					vkBuffer_ASScratch = VK_NULL_HANDLE;
					fprintf(gFILE, "uninitialize(): vkBuffer_ASScratch is freed\n");
					hasRTResources = true;
				}

				if (vkDeviceMemory_ASScratch != VK_NULL_HANDLE)
				{
					vkFreeMemory(vkDevice, vkDeviceMemory_ASScratch, NULL);
					vkDeviceMemory_ASScratch = VK_NULL_HANDLE;
					fprintf(gFILE, "uninitialize(): vkDeviceMemory_ASScratch is freed\n");
					hasRTResources = true;
				}

				// Destroy ground geometry buffers
				if (vkBuffer_GroundVertices != VK_NULL_HANDLE)
				{
					vkDestroyBuffer(vkDevice, vkBuffer_GroundVertices, NULL);
					vkBuffer_GroundVertices = VK_NULL_HANDLE;
					fprintf(gFILE, "uninitialize(): vkBuffer_GroundVertices is freed\n");
					hasRTResources = true;
				}

				if (vkDeviceMemory_GroundVertices != VK_NULL_HANDLE)
				{
					vkFreeMemory(vkDevice, vkDeviceMemory_GroundVertices, NULL);
					vkDeviceMemory_GroundVertices = VK_NULL_HANDLE;
					fprintf(gFILE, "uninitialize(): vkDeviceMemory_GroundVertices is freed\n");
					hasRTResources = true;
				}

				if (vkBuffer_GroundIndices != VK_NULL_HANDLE)
				{
					vkDestroyBuffer(vkDevice, vkBuffer_GroundIndices, NULL);
					vkBuffer_GroundIndices = VK_NULL_HANDLE;
					fprintf(gFILE, "uninitialize(): vkBuffer_GroundIndices is freed\n");
					hasRTResources = true;
				}

				if (vkDeviceMemory_GroundIndices != VK_NULL_HANDLE)
				{
					vkFreeMemory(vkDevice, vkDeviceMemory_GroundIndices, NULL);
					vkDeviceMemory_GroundIndices = VK_NULL_HANDLE;
					fprintf(gFILE, "uninitialize(): vkDeviceMemory_GroundIndices is freed\n");
					hasRTResources = true;
				}

				if (hasRTResources)
				{
					fprintf(gFILE, "uninitialize(): Ray Tracing cleanup complete\n");
				}
			}
...
VkResult FillDeviceExtensionNames(void)
{
	...
	*/
	...
	Accordingly set global variables, "required extension count" and "required extension names array"
	*/
	//https://registry.khronos.org/vulkan/specs/latest/man/html/VkBool32.html -> Vulkan cha bool
	VkBool32 vulkanSwapchainExtensionFound = VK_FALSE;

	// Ray tracing extension availability flags
	VkBool32 rtAccelerationStructureFound = VK_FALSE;
	VkBool32 rtRayQueryFound = VK_FALSE;
	VkBool32 rtBufferDeviceAddressFound = VK_FALSE;
	VkBool32 rtDeferredHostOpsFound = VK_FALSE;
	VkBool32 rtDescriptorIndexingFound = VK_FALSE;
	VkBool32 rtSpirv14Found = VK_FALSE;
	VkBool32 rtShaderFloatControlsFound = VK_FALSE;

	for (uint32_t i = 0; i < deviceExtensionCount; i++)
	{
		// Required extension: Swapchain
		...
		// Ray Tracing extensions (optional but all required for RT support)
		if (strcmp(deviceExtensionNames_array[i], VK_KHR_ACCELERATION_STRUCTURE_EXTENSION_NAME) == 0)
		{
			rtAccelerationStructureFound = VK_TRUE;
		}
		if (strcmp(deviceExtensionNames_array[i], VK_KHR_RAY_QUERY_EXTENSION_NAME) == 0)
		{
			rtRayQueryFound = VK_TRUE;
		}
		if (strcmp(deviceExtensionNames_array[i], VK_KHR_BUFFER_DEVICE_ADDRESS_EXTENSION_NAME) == 0)
		{
			rtBufferDeviceAddressFound = VK_TRUE;
		}
		if (strcmp(deviceExtensionNames_array[i], VK_KHR_DEFERRED_HOST_OPERATIONS_EXTENSION_NAME) == 0)
		{
			rtDeferredHostOpsFound = VK_TRUE;
		}
		if (strcmp(deviceExtensionNames_array[i], VK_EXT_DESCRIPTOR_INDEXING_EXTENSION_NAME) == 0)
		{
			rtDescriptorIndexingFound = VK_TRUE;
		}
		if (strcmp(deviceExtensionNames_array[i], VK_KHR_SPIRV_1_4_EXTENSION_NAME) == 0)
		{
			rtSpirv14Found = VK_TRUE;
		}
		if (strcmp(deviceExtensionNames_array[i], VK_KHR_SHADER_FLOAT_CONTROLS_EXTENSION_NAME) == 0)
		{
			rtShaderFloatControlsFound = VK_TRUE;
		}
	}

	// Check if all ray tracing extensions are available
	if (rtAccelerationStructureFound && rtRayQueryFound && rtBufferDeviceAddressFound &&
	    rtDeferredHostOpsFound && rtDescriptorIndexingFound && rtSpirv14Found && rtShaderFloatControlsFound)
	{
		fprintf(gFILE, "FillDeviceExtensionNames(): All Ray Tracing extensions found - enabling RT support\n");
		bRayTracingSupported = TRUE;

		// Add ray tracing extensions
		enabledDeviceExtensionNames_array[enabledDeviceExtensionsCount++] = VK_KHR_ACCELERATION_STRUCTURE_EXTENSION_NAME;
		enabledDeviceExtensionNames_array[enabledDeviceExtensionsCount++] = VK_KHR_RAY_QUERY_EXTENSION_NAME;
		enabledDeviceExtensionNames_array[enabledDeviceExtensionsCount++] = VK_KHR_BUFFER_DEVICE_ADDRESS_EXTENSION_NAME;
		enabledDeviceExtensionNames_array[enabledDeviceExtensionsCount++] = VK_KHR_DEFERRED_HOST_OPERATIONS_EXTENSION_NAME;
		enabledDeviceExtensionNames_array[enabledDeviceExtensionsCount++] = VK_EXT_DESCRIPTOR_INDEXING_EXTENSION_NAME;
		enabledDeviceExtensionNames_array[enabledDeviceExtensionsCount++] = VK_KHR_SPIRV_1_4_EXTENSION_NAME;
		enabledDeviceExtensionNames_array[enabledDeviceExtensionsCount++] = VK_KHR_SHADER_FLOAT_CONTROLS_EXTENSION_NAME;
	}
	else
	{
		fprintf(gFILE, "FillDeviceExtensionNames(): Ray Tracing extensions not fully supported - RT disabled\n");
		fprintf(gFILE, "  VK_KHR_ACCELERATION_STRUCTURE: %s\n", rtAccelerationStructureFound ? "YES" : "NO");
		fprintf(gFILE, "  VK_KHR_RAY_QUERY: %s\n", rtRayQueryFound ? "YES" : "NO");
		fprintf(gFILE, "  VK_KHR_BUFFER_DEVICE_ADDRESS: %s\n", rtBufferDeviceAddressFound ? "YES" : "NO");
		fprintf(gFILE, "  VK_KHR_DEFERRED_HOST_OPERATIONS: %s\n", rtDeferredHostOpsFound ? "YES" : "NO");
		fprintf(gFILE, "  VK_EXT_DESCRIPTOR_INDEXING: %s\n", rtDescriptorIndexingFound ? "YES" : "NO");
		fprintf(gFILE, "  VK_KHR_SPIRV_1_4: %s\n", rtSpirv14Found ? "YES" : "NO");
		fprintf(gFILE, "  VK_KHR_SHADER_FLOAT_CONTROLS: %s\n", rtShaderFloatControlsFound ? "YES" : "NO");
		fprintf(gFILE, "FillDeviceExtensionNames(): Ray tracing required but not available\n");
		vkResult = VK_ERROR_INITIALIZATION_FAILED;
		return vkResult;
	}
...
VkResult CreateVulKanDevice(void)
{
	...
	*/
	...
	// ============================================================================
	// RAY TRACING FEATURE ENABLING
	// Chain feature structures to enable ray tracing when supported
	// ============================================================================
	...
	if (bRayTracingSupported)
	{
		fprintf(gFILE, "CreateVulKanDevice(): Setting up Ray Tracing feature structures\n");

		// Query supported features from the physical device first
		VkPhysicalDeviceDescriptorIndexingFeatures supportedDescriptorIndexing = {};
		supportedDescriptorIndexing.sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_DESCRIPTOR_INDEXING_FEATURES;
		supportedDescriptorIndexing.pNext = NULL;

		VkPhysicalDeviceBufferDeviceAddressFeatures supportedBufferAddress = {};
		supportedBufferAddress.sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_BUFFER_DEVICE_ADDRESS_FEATURES;
		supportedBufferAddress.pNext = &supportedDescriptorIndexing;
		...
		supportedAccelStruct.pNext = &supportedBufferAddress;
		...
		supportedRayQuery.pNext = &supportedAccelStruct;

		VkPhysicalDeviceFeatures2 deviceFeatures2 = {};
		deviceFeatures2.sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_FEATURES_2;
		deviceFeatures2.pNext = &supportedRayQuery;

		vkGetPhysicalDeviceFeatures2(vkPhysicalDevice_selected, &deviceFeatures2);
		fprintf(gFILE, "CreateVulKanDevice(): Queried physical device features\n");

		// Verify required features are supported
		if (!supportedBufferAddress.bufferDeviceAddress ||
		    !supportedAccelStruct.accelerationStructure ||
		    !supportedRayQuery.rayQuery)
		{
			fprintf(gFILE, "CreateVulKanDevice(): Required RT features not supported by device, disabling ray tracing\n");
			fprintf(gFILE, "  bufferDeviceAddress: %s\n", supportedBufferAddress.bufferDeviceAddress ? "YES" : "NO");
			fprintf(gFILE, "  accelerationStructure: %s\n", supportedAccelStruct.accelerationStructure ? "YES" : "NO");
			fprintf(gFILE, "  rayQuery: %s\n", supportedRayQuery.rayQuery ? "YES" : "NO");
			bRayTracingSupported = FALSE;
		}
		else
		{
			// Descriptor indexing features - only enable what the device supports
			descriptorIndexingFeatures.sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_DESCRIPTOR_INDEXING_FEATURES;
			descriptorIndexingFeatures.pNext = NULL;
			descriptorIndexingFeatures.shaderSampledImageArrayNonUniformIndexing = supportedDescriptorIndexing.shaderSampledImageArrayNonUniformIndexing;
			descriptorIndexingFeatures.runtimeDescriptorArray = supportedDescriptorIndexing.runtimeDescriptorArray;
			descriptorIndexingFeatures.descriptorBindingPartiallyBound = supportedDescriptorIndexing.descriptorBindingPartiallyBound;
			descriptorIndexingFeatures.descriptorBindingVariableDescriptorCount = supportedDescriptorIndexing.descriptorBindingVariableDescriptorCount;

			fprintf(gFILE, "CreateVulKanDevice(): Descriptor indexing features:\n");
			fprintf(gFILE, "  shaderSampledImageArrayNonUniformIndexing: %s\n", descriptorIndexingFeatures.shaderSampledImageArrayNonUniformIndexing ? "YES" : "NO");
			fprintf(gFILE, "  runtimeDescriptorArray: %s\n", descriptorIndexingFeatures.runtimeDescriptorArray ? "YES" : "NO");
			fprintf(gFILE, "  descriptorBindingPartiallyBound: %s\n", descriptorIndexingFeatures.descriptorBindingPartiallyBound ? "YES" : "NO");
			fprintf(gFILE, "  descriptorBindingVariableDescriptorCount: %s\n", descriptorIndexingFeatures.descriptorBindingVariableDescriptorCount ? "YES" : "NO");

			// Buffer device address feature (required for acceleration structures)
			bufferDeviceAddressFeatures.sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_BUFFER_DEVICE_ADDRESS_FEATURES;
			bufferDeviceAddressFeatures.pNext = &descriptorIndexingFeatures;
			bufferDeviceAddressFeatures.bufferDeviceAddress = VK_TRUE;

			// Acceleration structure feature (core RT feature)
			accelerationStructureFeatures.sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_ACCELERATION_STRUCTURE_FEATURES_KHR;
			accelerationStructureFeatures.pNext = &bufferDeviceAddressFeatures;
			accelerationStructureFeatures.accelerationStructure = VK_TRUE;

			// Ray query feature (allows ray queries in compute/fragment shaders)
			rayQueryFeatures.sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_RAY_QUERY_FEATURES_KHR;
			rayQueryFeatures.pNext = &accelerationStructureFeatures;
			rayQueryFeatures.rayQuery = VK_TRUE;

			// Chain the features to device creation
			vkDeviceCreateInfo.pNext = &rayQueryFeatures;

			fprintf(gFILE, "CreateVulKanDevice(): Ray Tracing features chained to device creation\n");
		}
	}
	...
	*/
	...
	// These must be loaded dynamically as they are extension functions
	// ============================================================================
	if (bRayTracingSupported)
	{
		fprintf(gFILE, "CreateVulKanDevice(): Loading Ray Tracing function pointers\n");

		fpGetBufferDeviceAddressKHR = (PFN_vkGetBufferDeviceAddressKHR)
			vkGetDeviceProcAddr(vkDevice, "vkGetBufferDeviceAddressKHR");
		fpCreateAccelerationStructureKHR = (PFN_vkCreateAccelerationStructureKHR)
			vkGetDeviceProcAddr(vkDevice, "vkCreateAccelerationStructureKHR");
		fpDestroyAccelerationStructureKHR = (PFN_vkDestroyAccelerationStructureKHR)
			vkGetDeviceProcAddr(vkDevice, "vkDestroyAccelerationStructureKHR");
		fpGetAccelerationStructureBuildSizesKHR = (PFN_vkGetAccelerationStructureBuildSizesKHR)
			vkGetDeviceProcAddr(vkDevice, "vkGetAccelerationStructureBuildSizesKHR");
		fpCmdBuildAccelerationStructuresKHR = (PFN_vkCmdBuildAccelerationStructuresKHR)
			vkGetDeviceProcAddr(vkDevice, "vkCmdBuildAccelerationStructuresKHR");
		fpGetAccelerationStructureDeviceAddressKHR = (PFN_vkGetAccelerationStructureDeviceAddressKHR)
			vkGetDeviceProcAddr(vkDevice, "vkGetAccelerationStructureDeviceAddressKHR");

		// Verify all function pointers were loaded
		if (fpGetBufferDeviceAddressKHR && fpCreateAccelerationStructureKHR &&
		    fpDestroyAccelerationStructureKHR && fpGetAccelerationStructureBuildSizesKHR &&
		    fpCmdBuildAccelerationStructuresKHR && fpGetAccelerationStructureDeviceAddressKHR)
		{
			fprintf(gFILE, "CreateVulKanDevice(): All Ray Tracing function pointers loaded successfully\n");
		}
		else
		{
			fprintf(gFILE, "CreateVulKanDevice(): Failed to load Ray Tracing function pointers - ray tracing required\n");
			return VK_ERROR_INITIALIZATION_FAILED;
		}
		...
		// Get ray tracing properties
		{
			...
			rtAccelStructProperties.sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_ACCELERATION_STRUCTURE_PROPERTIES_KHR;
			rtAccelStructProperties.pNext = &rtPipelineProperties;

			VkPhysicalDeviceProperties2 deviceProperties2 = {};
			deviceProperties2.sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_PROPERTIES_2;
			deviceProperties2.pNext = &rtAccelStructProperties;

			vkGetPhysicalDeviceProperties2(vkPhysicalDevice_selected, &deviceProperties2);

			fprintf(gFILE, "CreateVulKanDevice(): RT Properties - maxGeometryCount: %llu\n",
				rtAccelStructProperties.maxGeometryCount);
			fprintf(gFILE, "CreateVulKanDevice(): RT Properties - maxInstanceCount: %llu\n",
				rtAccelStructProperties.maxInstanceCount);
		}
	}
...
VkResult CreateSwapChain(VkBool32 vsync)
{
	...
	*/
	...
	// Ensure minimum extent of 1x1 to prevent zero-sized swapchain (can happen during fullscreen transitions)
	if(vkExtent2D_SwapChain.width < 1)
	{
		vkExtent2D_SwapChain.width = 1;
		fprintf(gFILE, "CreateSwapChain(): Clamped width to minimum of 1\n");
	}
	if(vkExtent2D_SwapChain.height < 1)
	{
		vkExtent2D_SwapChain.height = 1;
		fprintf(gFILE, "CreateSwapChain(): Clamped height to minimum of 1\n");
	}
...
VkResult CreateVertexBuffer(void)
{
	...
	*/
	// FULLSCREEN FRONT FACE QUAD
	// position
	float cubeVertices[] =
	{
		// front face only (fullscreen quad)
		// triangle one
	...
	*/
	...
	if (vkResult != VK_SUCCESS)
	{
		fprintf(gFILE, "CreateVertexBuffer(): vkMapMemory() function failed for vertexdata_position.vkBuffer with error code %d\n", vkResult);
		return vkResult;
	}
	else
	{
		fprintf(gFILE, "CreateVertexBuffer(): vkMapMemory() succedded for vertexdata_position.vkBuffer\n");
	}
...
VkResult CreateTexture(const char* textureFileName)
{
	...
	if(fp == NULL)
	{
		fprintf(gFILE, "CreateTexture(): failed to open textureFileName %s for reading\n",textureFileName);
		return VK_ERROR_INITIALIZATION_FAILED;
	}
	...
	{
		fprintf(gFILE, "CreateTexture(): stbi_load_from_file() failed to get Image Info for %s\n",textureFileName);
		return VK_ERROR_INITIALIZATION_FAILED;
	}
	...
	*/
	...
	if (vkResult != VK_SUCCESS)
	{
		fprintf(gFILE, "CreateTexture(): vkCreateBuffer() function for creating Staging Buffer failed with error code %d\n", vkResult);
		return vkResult;
	}
	else
	{
		fprintf(gFILE, "CreateTexture(): vkCreateBuffer() for creating staging Buffer succedded\n");
	}
	...
	*/
	...
	if (vkResult != VK_SUCCESS)
	{
		fprintf(gFILE, "CreateTexture(): vkAllocateMemory() for allocating staging Buffer device memory failed with error code %d\n", vkResult);
		return vkResult;
	}
	else
	{
		fprintf(gFILE, "CreateTexture(): vkAllocateMemory() for allocating staging Buffer device memory succedded\n");
	}
	...
	*/
	...
	if (vkResult != VK_SUCCESS)
	{
		fprintf(gFILE, "CreateTexture(): vkBindBufferMemory() for staging buffer failed with error code %d\n", vkResult);
		return vkResult;
	}
	else
	{
		fprintf(gFILE, "CreateTexture(): vkBindBufferMemory() for staging buffer succedded\n");
	}
	...
	*/
	...
	if (vkResult != VK_SUCCESS)
	{
		fprintf(gFILE, "CreateTexture(): vkMapMemory() for staging buffer failed with error code %d\n", vkResult);
		return vkResult;
	}
	else
	{
		fprintf(gFILE, "CreateTexture(): vkMapMemory() for staging buffer succedded\n");
	}
	...
	*/
	...
	imageData = NULL;
	fprintf(gFILE, "CreateTexture(): stbi_image_free freed imageData\n");
	...
	*/
	...
	if (vkResult != VK_SUCCESS)
	{
		fprintf(gFILE, "CreateTexture(): vkCreateImage() function failed with error code %d\n", vkResult);
		return vkResult;
	}
	else
	{
		fprintf(gFILE, "CreateTexture(): vkCreateImage() succedded\n");
	}
	...
	*/
	...
	{
		fprintf(gFILE, "CreateTexture(): Unsupported texture layout transition\n");
		return VK_ERROR_INITIALIZATION_FAILED;
	}
	...
	*/
	...
	if (vkResult != VK_SUCCESS)
	{
		fprintf(gFILE, "CreateTexture(): vkEndCommandBuffer() for vkCommandBufferBeginInfo_transition_image_layout failed with error code %d\n", vkResult);
		return vkResult;
	}
	else
	{
		fprintf(gFILE, "CreateTexture(): vkEndCommandBuffer() for vkCommandBufferBeginInfo_transition_image_layout succedded\n");
	}
	...
	if(vkResult != VK_SUCCESS)
	{
		fprintf(gFILE, "CreateTexture(): vkQueueSubmit()  for vkSubmitInfo_transition_image_layout failed\n");
		return vkResult;
	}
	else
	{
		fprintf(gFILE, "CreateTexture(): vkQueueSubmit()  for vkSubmitInfo_transition_image_layout suceeded\n");
	}
	...
	*/
	...
	if(vkResult != VK_SUCCESS)
	{
		fprintf(gFILE, "CreateTexture(): vkQueueWaitIdle()  for vkSubmitInfo_transition_image_layout failed\n");
		return vkResult;
	}
	else
	{
		fprintf(gFILE, "CreateTexture(): vkQueueWaitIdle()  for vkSubmitInfo_transition_image_layout suceeded\n");
	}
	...
	*/
	...
	if (vkResult != VK_SUCCESS)
	{
		fprintf(gFILE, "CreateTexture(): vkAllocateCommandBuffers() function for vkCommandBuffer_buffer_to_image_copy failed with error code %d\n", vkResult);
		return vkResult;
	}
	else
	{
		fprintf(gFILE, "CreateTexture(): vkAllocateCommandBuffers() succedded for vkCommandBuffer_buffer_to_image_copyn");
	}
	...
	*/
	...
	if (vkResult != VK_SUCCESS)
	{
		fprintf(gFILE, "CreateTexture(): vkBeginCommandBuffer() for vkCommandBufferBeginInfo_buffer_to_image_copy copy failed with error code %d\n", vkResult);
		return vkResult;
	}
	else
	{
		fprintf(gFILE, "CreateTexture(): vkBeginCommandBuffer() for vkCommandBufferBeginInfo_buffer_to_image_copy copy succedded\n");
	}
	...
	*/
	...
	if (vkResult != VK_SUCCESS)
	{
		fprintf(gFILE, "CreateTexture(): vkEndCommandBuffer() for vkCommandBufferBeginInfo_buffer_to_image_copy copy failed with error code %d\n", vkResult);
		return vkResult;
	}
	else
	{
		fprintf(gFILE, "CreateTexture(): vkEndCommandBuffer() for vkCommandBufferBeginInfo_buffer_to_image_copy copy succedded\n");
	}
	...
	if(vkResult != VK_SUCCESS)
	{
		fprintf(gFILE, "CreateTexture(): vkQueueSubmit()  for vkSubmitInfo_buffer_to_image_copy failed\n");
		return vkResult;
	}
	else
	{
		fprintf(gFILE, "CreateTexture(): vkQueueSubmit()  for vkSubmitInfo_buffer_to_image_copy suceeded\n");
	}
	...
	*/
	...
	if(vkResult != VK_SUCCESS)
	{
		fprintf(gFILE, "CreateTexture(): vkQueueWaitIdle()  for vkSubmitInfo_buffer_to_image_copy failed\n");
		return vkResult;
	}
	else
	{
		fprintf(gFILE, "CreateTexture(): vkQueueWaitIdle()  for vkSubmitInfo_buffer_to_image_copy suceeded\n");
	}
...
// ==================== COMPUTE SHADER FUNCTIONS FOR PROCEDURAL CLOUDS ====================
...
VkResult CreateComputeStorageImage(void)
{
	...
	imageCreateInfo.extent.width = vkExtent2D_SwapChain.width;
	imageCreateInfo.extent.height = vkExtent2D_SwapChain.height;
	imageCreateInfo.extent.depth = 1;
	imageCreateInfo.mipLevels = 1;
	imageCreateInfo.arrayLayers = 1;
	imageCreateInfo.samples = VK_SAMPLE_COUNT_1_BIT;
	imageCreateInfo.tiling = VK_IMAGE_TILING_OPTIMAL;
	// Need TRANSFER_SRC for copying to history buffer (temporal reprojection)
	imageCreateInfo.usage = VK_IMAGE_USAGE_STORAGE_BIT | VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_TRANSFER_SRC_BIT;
	imageCreateInfo.sharingMode = VK_SHARING_MODE_EXCLUSIVE;
	imageCreateInfo.initialLayout = VK_IMAGE_LAYOUT_UNDEFINED;

	vkResult = vkCreateImage(vkDevice, &imageCreateInfo, NULL, &vkImage_computeOutput);
	if(vkResult != VK_SUCCESS)
	{
		fprintf(gFILE, "CreateComputeStorageImage(): vkCreateImage() failed with error code %d\n", vkResult);
		return vkResult;
	}
	fprintf(gFILE, "CreateComputeStorageImage(): vkCreateImage() succeeded\n");

	// Get memory requirements
	VkMemoryRequirements memRequirements;
	vkGetImageMemoryRequirements(vkDevice, vkImage_computeOutput, &memRequirements);
	...
	vkResult = vkAllocateMemory(vkDevice, &allocInfo, NULL, &vkDeviceMemory_computeOutput);
	if(vkResult != VK_SUCCESS)
	{
		fprintf(gFILE, "CreateComputeStorageImage(): vkAllocateMemory() failed with error code %d\n", vkResult);
		return vkResult;
	}
	fprintf(gFILE, "CreateComputeStorageImage(): vkAllocateMemory() succeeded\n");

	vkResult = vkBindImageMemory(vkDevice, vkImage_computeOutput, vkDeviceMemory_computeOutput, 0);
	if(vkResult != VK_SUCCESS)
	{
		fprintf(gFILE, "CreateComputeStorageImage(): vkBindImageMemory() failed with error code %d\n", vkResult);
		return vkResult;
	}
	fprintf(gFILE, "CreateComputeStorageImage(): vkBindImageMemory() succeeded\n");

	// Create image view
	VkImageViewCreateInfo viewCreateInfo;
	memset(&viewCreateInfo, 0, sizeof(VkImageViewCreateInfo));
	viewCreateInfo.sType = VK_STRUCTURE_TYPE_IMAGE_VIEW_CREATE_INFO;
	viewCreateInfo.image = vkImage_computeOutput;
	...
	vkResult = vkCreateImageView(vkDevice, &viewCreateInfo, NULL, &vkImageView_computeOutput);
	if(vkResult != VK_SUCCESS)
	{
		fprintf(gFILE, "CreateComputeStorageImage(): vkCreateImageView() failed with error code %d\n", vkResult);
		return vkResult;
	}
	fprintf(gFILE, "CreateComputeStorageImage(): vkCreateImageView() succeeded\n");

	// Transition image layout to GENERAL for compute shader
	VkCommandBuffer cmdBuffer;
	VkCommandBufferAllocateInfo cmdAllocInfo;
	memset(&cmdAllocInfo, 0, sizeof(VkCommandBufferAllocateInfo));
	...
	barrier.image = vkImage_computeOutput;
	...
	fprintf(gFILE, "CreateComputeStorageImage(): Image layout transitioned to GENERAL\n");

	return vkResult;
}

// ============================================================================
// TEMPORAL REPROJECTION - History Buffer Creation
// Creates a storage image to hold the previous frame for temporal accumulation
// ============================================================================
VkResult CreateTemporalHistoryBuffer(void)
{
	VkResult vkResult = VK_SUCCESS;

	// Create history image with same format as compute output
	VkImageCreateInfo imageCreateInfo;
	memset(&imageCreateInfo, 0, sizeof(VkImageCreateInfo));
	imageCreateInfo.sType = VK_STRUCTURE_TYPE_IMAGE_CREATE_INFO;
	imageCreateInfo.imageType = VK_IMAGE_TYPE_2D;
	...
	imageCreateInfo.samples = VK_SAMPLE_COUNT_1_BIT;
	imageCreateInfo.tiling = VK_IMAGE_TILING_OPTIMAL;
	// Need both SAMPLED (for reading in shader) and TRANSFER_DST (for copying from output)
	imageCreateInfo.usage = VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT;
	imageCreateInfo.sharingMode = VK_SHARING_MODE_EXCLUSIVE;
	imageCreateInfo.initialLayout = VK_IMAGE_LAYOUT_UNDEFINED;

	vkResult = vkCreateImage(vkDevice, &imageCreateInfo, NULL, &vkImage_historyBuffer);
	if(vkResult != VK_SUCCESS)
	{
		fprintf(gFILE, "CreateTemporalHistoryBuffer(): vkCreateImage() failed with error code %d\n", vkResult);
		return vkResult;
	}
	fprintf(gFILE, "CreateTemporalHistoryBuffer(): vkCreateImage() succeeded\n");

	// Get memory requirements
	VkMemoryRequirements memRequirements;
	vkGetImageMemoryRequirements(vkDevice, vkImage_historyBuffer, &memRequirements);
	...
	vkResult = vkAllocateMemory(vkDevice, &allocInfo, NULL, &vkDeviceMemory_historyBuffer);
	if(vkResult != VK_SUCCESS)
	{
		fprintf(gFILE, "CreateTemporalHistoryBuffer(): vkAllocateMemory() failed with error code %d\n", vkResult);
		return vkResult;
	}
	fprintf(gFILE, "CreateTemporalHistoryBuffer(): vkAllocateMemory() succeeded\n");

	vkResult = vkBindImageMemory(vkDevice, vkImage_historyBuffer, vkDeviceMemory_historyBuffer, 0);
	if(vkResult != VK_SUCCESS)
	{
		fprintf(gFILE, "CreateTemporalHistoryBuffer(): vkBindImageMemory() failed with error code %d\n", vkResult);
		return vkResult;
	}
	fprintf(gFILE, "CreateTemporalHistoryBuffer(): vkBindImageMemory() succeeded\n");

	// Create image view
	VkImageViewCreateInfo viewCreateInfo;
	memset(&viewCreateInfo, 0, sizeof(VkImageViewCreateInfo));
	viewCreateInfo.sType = VK_STRUCTURE_TYPE_IMAGE_VIEW_CREATE_INFO;
	viewCreateInfo.image = vkImage_historyBuffer;
	viewCreateInfo.viewType = VK_IMAGE_VIEW_TYPE_2D;
	viewCreateInfo.format = VK_FORMAT_R8G8B8A8_UNORM;
	viewCreateInfo.subresourceRange.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT;
	viewCreateInfo.subresourceRange.baseMipLevel = 0;
	...
	vkResult = vkCreateImageView(vkDevice, &viewCreateInfo, NULL, &vkImageView_historyBuffer);
	if(vkResult != VK_SUCCESS)
	{
		fprintf(gFILE, "CreateTemporalHistoryBuffer(): vkCreateImageView() failed with error code %d\n", vkResult);
		return vkResult;
	}
	fprintf(gFILE, "CreateTemporalHistoryBuffer(): vkCreateImageView() succeeded\n");

	// Transition image layout to SHADER_READ_ONLY_OPTIMAL for sampling
	VkCommandBuffer cmdBuffer;
	VkCommandBufferAllocateInfo cmdAllocInfo;
	memset(&cmdAllocInfo, 0, sizeof(VkCommandBufferAllocateInfo));
	cmdAllocInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO;
	...
	barrier.srcQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED;
	barrier.dstQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED;
	barrier.image = vkImage_historyBuffer;
	barrier.subresourceRange.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT;
	barrier.subresourceRange.baseMipLevel = 0;
	barrier.subresourceRange.levelCount = 1;
	barrier.subresourceRange.baseArrayLayer = 0;
	...
	fprintf(gFILE, "CreateTemporalHistoryBuffer(): Image layout transitioned to SHADER_READ_ONLY_OPTIMAL\n");

	// Reset temporal frame index since we have a new history buffer
	temporalFrameIndex = 0;
...
VkResult CreateHistorySampler(void)
{
	...
	samplerInfo.magFilter = VK_FILTER_LINEAR;  // Bilinear for smooth reprojection
	...
	vkResult = vkCreateSampler(vkDevice, &samplerInfo, NULL, &vkSampler_history);
	if(vkResult != VK_SUCCESS)
	{
		fprintf(gFILE, "CreateHistorySampler(): vkCreateSampler() failed with error code %d\n", vkResult);
		return vkResult;
	}
	fprintf(gFILE, "CreateHistorySampler(): vkCreateSampler() succeeded\n");
...
VkResult CreateCloudParamsUniformBuffer(void)
{
	...
	bufferCreateInfo.size = sizeof(CloudParamsData);
	bufferCreateInfo.usage = VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT;
	bufferCreateInfo.sharingMode = VK_SHARING_MODE_EXCLUSIVE;

	vkResult = vkCreateBuffer(vkDevice, &bufferCreateInfo, NULL, &cloudParamsBuffer.vkBuffer);
	if(vkResult != VK_SUCCESS)
	{
		fprintf(gFILE, "CreateCloudParamsUniformBuffer(): vkCreateBuffer() failed with error code %d\n", vkResult);
		return vkResult;
	}
	fprintf(gFILE, "CreateCloudParamsUniformBuffer(): vkCreateBuffer() succeeded\n");

	VkMemoryRequirements memRequirements;
	vkGetBufferMemoryRequirements(vkDevice, cloudParamsBuffer.vkBuffer, &memRequirements);
	...
	vkResult = vkAllocateMemory(vkDevice, &allocInfo, NULL, &cloudParamsBuffer.vkDeviceMemory);
	if(vkResult != VK_SUCCESS)
	{
		fprintf(gFILE, "CreateCloudParamsUniformBuffer(): vkAllocateMemory() failed with error code %d\n", vkResult);
		return vkResult;
	}
	fprintf(gFILE, "CreateCloudParamsUniformBuffer(): vkAllocateMemory() succeeded\n");

	vkResult = vkBindBufferMemory(vkDevice, cloudParamsBuffer.vkBuffer, cloudParamsBuffer.vkDeviceMemory, 0);
	if(vkResult != VK_SUCCESS)
	{
		fprintf(gFILE, "CreateCloudParamsUniformBuffer(): vkBindBufferMemory() failed with error code %d\n", vkResult);
		return vkResult;
	}
	fprintf(gFILE, "CreateCloudParamsUniformBuffer(): vkBindBufferMemory() succeeded\n");

	// Initialize cloud params with fixed view position and vectors
	// Camera tilted upward to position cloud in upper viewport
	glm::vec3 fixedPosition = glm::vec3(0.0f, 5.0f, 30.0f);
	float upwardTilt = glm::radians(20.0f);  // 20 degrees upward tilt
	glm::vec3 fixedForward = glm::normalize(glm::vec3(0.0f, sin(upwardTilt), -cos(upwardTilt)));
	glm::vec3 fixedRight = glm::vec3(1.0f, 0.0f, 0.0f);
	glm::vec3 fixedUp = glm::normalize(glm::cross(fixedRight, fixedForward));
	float fixedFov = 60.0f;

	CloudParamsData cloudParams;
	cloudParams.cameraPos = glm::vec4(fixedPosition, 0.0f);
	cloudParams.sunDirection = glm::vec4(glm::normalize(glm::vec3(0.5f, 0.8f, 0.3f)), 0.0f);
	cloudParams.screenSize = glm::vec4((float)vkExtent2D_SwapChain.width, (float)vkExtent2D_SwapChain.height, 0.0f, 0.0f);
	cloudParams.cameraForward = glm::vec4(fixedForward, 0.0f);
	cloudParams.cameraRight = glm::vec4(fixedRight, 0.0f);
	cloudParams.cameraUp = glm::vec4(fixedUp, glm::radians(fixedFov));
	// Initialize temporal reprojection data (first frame - no blending)
	cloudParams.prevCameraPos = glm::vec4(fixedPosition, 0.0f);  // frame index = 0
	cloudParams.prevCameraForward = glm::vec4(fixedForward, 0.0f);  // blend factor = 0
	cloudParams.prevCameraRight = glm::vec4(fixedRight, 0.0f);
	cloudParams.prevCameraUp = glm::vec4(fixedUp, glm::radians(fixedFov));

	void* data;
	vkMapMemory(vkDevice, cloudParamsBuffer.vkDeviceMemory, 0, sizeof(CloudParamsData), 0, &data);
	memcpy(data, &cloudParams, sizeof(CloudParamsData));
	vkUnmapMemory(vkDevice, cloudParamsBuffer.vkDeviceMemory);

	fprintf(gFILE, "CreateCloudParamsUniformBuffer(): Cloud params initialized with fixed view vectors\n");
...
VkResult UpdateCloudParamsBuffer(void)
{
	...
	cloudTime += 0.016f;  // Approximate 60 FPS delta time

	// Fixed view position and vectors
	// Camera tilted upward to position cloud in upper viewport
	glm::vec3 fixedPosition = glm::vec3(0.0f, 5.0f, 30.0f);
	float upwardTilt = glm::radians(20.0f);  // 20 degrees upward tilt
	glm::vec3 fixedForward = glm::normalize(glm::vec3(0.0f, sin(upwardTilt), -cos(upwardTilt)));
	...
	// Static view - always increase temporal blending
	// Calculate blend factor BEFORE incrementing frame index
	// This ensures first frame has blend factor = 0, allowing history buffer to be filled first
	float maxBlend = 0.95f;
	float blendRate = 0.1f;
	float blendFactor = maxBlend * (1.0f - exp(-blendRate * (float)temporalFrameIndex));
	temporalFrameIndex++;

	CloudParamsData cloudParams;
	cloudParams.cameraPos = glm::vec4(fixedPosition, cloudTime);
	cloudParams.sunDirection = glm::vec4(glm::normalize(glm::vec3(0.5f, 0.8f, 0.3f)), 0.0f);
	cloudParams.screenSize = glm::vec4((float)vkExtent2D_SwapChain.width, (float)vkExtent2D_SwapChain.height, 0.0f, 0.0f);
	...
	// Temporal reprojection data (static view)
	cloudParams.prevCameraPos = glm::vec4(fixedPosition, (float)temporalFrameIndex);
	cloudParams.prevCameraForward = glm::vec4(fixedForward, blendFactor);
	cloudParams.prevCameraRight = glm::vec4(fixedRight, 0.0f);
	cloudParams.prevCameraUp = glm::vec4(fixedUp, glm::radians(fixedFov));

	void* data;
	vkResult = vkMapMemory(vkDevice, cloudParamsBuffer.vkDeviceMemory, 0, sizeof(CloudParamsData), 0, &data);
	if(vkResult == VK_SUCCESS)
	{
		memcpy(data, &cloudParams, sizeof(CloudParamsData));
		vkUnmapMemory(vkDevice, cloudParamsBuffer.vkDeviceMemory);
...
VkResult CreateComputeShader(void)
{
	...
	const char* szFileName = "CloudComputeRT.comp.spv";
	FILE* fp = fopen(szFileName, "rb");
	if(fp == NULL)
	{
		fprintf(gFILE, "CreateComputeShader(): Failed to open compute shader SPIRV file\n");
		return VK_ERROR_INITIALIZATION_FAILED;
	}
	fprintf(gFILE, "CreateComputeShader(): Opened compute shader SPIRV file\n");

	fseek(fp, 0L, SEEK_END);
	size_t size = ftell(fp);
	if(size == 0)
	{
		fprintf(gFILE, "CreateComputeShader(): Compute shader SPIRV file size is 0\n");
		fclose(fp);
		return VK_ERROR_INITIALIZATION_FAILED;
	}
	...
	if(shaderData == NULL)
	{
		fprintf(gFILE, "CreateComputeShader(): malloc for compute shader SPIRV file failed\n");
		fclose(fp);
		return VK_ERROR_INITIALIZATION_FAILED;
	}
	...
	if(retVal != 1)
	{
		fprintf(gFILE, "CreateComputeShader(): Failed to read compute shader SPIRV file\n");
		free(shaderData);
		fclose(fp);
		return VK_ERROR_INITIALIZATION_FAILED;
	}
	...
	vkResult = vkCreateShaderModule(vkDevice, &createInfo, NULL, &vkShaderModule_compute);
	if(vkResult != VK_SUCCESS)
	{
		fprintf(gFILE, "CreateComputeShader(): vkCreateShaderModule() failed with error code %d\n", vkResult);
		free(shaderData);
		return vkResult;
	}
	fprintf(gFILE, "CreateComputeShader(): vkCreateShaderModule() succeeded\n");
...
VkResult CreateComputeDescriptorSetLayout(void)
{
	...
	// Binding 1: Uniform buffer (cloud params)
	// Binding 2: Combined image sampler (history buffer for temporal reprojection)
	// Binding 3: Acceleration structure (for ray tracing - optional)
	...
	// Uniform buffer (cloud params with temporal data)
	bindings[1].binding = 1;
	bindings[1].descriptorType = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
	bindings[1].descriptorCount = 1;
	bindings[1].stageFlags = VK_SHADER_STAGE_COMPUTE_BIT;

	// History buffer sampler (for temporal reprojection)
	...
	// Ray tracing acceleration structure (binding 3) - only if RT supported
	if (bRayTracingSupported)
	{
		...
		bindingCount = 4;
		fprintf(gFILE, "CreateComputeDescriptorSetLayout(): Including acceleration structure binding (RT enabled)\n");
	}
	...
	vkResult = vkCreateDescriptorSetLayout(vkDevice, &layoutInfo, NULL, &vkDescriptorSetLayout_compute);
	if(vkResult != VK_SUCCESS)
	{
		fprintf(gFILE, "CreateComputeDescriptorSetLayout(): vkCreateDescriptorSetLayout() failed with error code %d\n", vkResult);
		return vkResult;
	}
	fprintf(gFILE, "CreateComputeDescriptorSetLayout(): vkCreateDescriptorSetLayout() succeeded with %d bindings\n", bindingCount);
...
VkResult CreateComputePipelineLayout(void)
{
	...
	pipelineLayoutInfo.pSetLayouts = &vkDescriptorSetLayout_compute;

	vkResult = vkCreatePipelineLayout(vkDevice, &pipelineLayoutInfo, NULL, &vkPipelineLayout_compute);
	if(vkResult != VK_SUCCESS)
	{
		fprintf(gFILE, "CreateComputePipelineLayout(): vkCreatePipelineLayout() failed with error code %d\n", vkResult);
		return vkResult;
	}
	fprintf(gFILE, "CreateComputePipelineLayout(): vkCreatePipelineLayout() succeeded\n");
...
VkResult CreateComputePipeline(void)
{
	...
	shaderStageInfo.module = vkShaderModule_compute;
	...
	vkResult = vkCreateComputePipelines(vkDevice, VK_NULL_HANDLE, 1, &pipelineInfo, NULL, &vkPipeline_compute);
	if(vkResult != VK_SUCCESS)
	{
		fprintf(gFILE, "CreateComputePipeline(): vkCreateComputePipelines() failed with error code %d\n", vkResult);
		return vkResult;
	}
	fprintf(gFILE, "CreateComputePipeline(): vkCreateComputePipelines() succeeded\n");
...
VkResult CreateComputeDescriptorPool(void)
{
	...
	// Pool sizes for: storage image (output), uniform buffer, combined image sampler (history)
	// and optionally acceleration structure (for ray tracing)
	...
	// Add acceleration structure pool size if RT supported
	if (bRayTracingSupported)
	{
		poolSizes[3].type = VK_DESCRIPTOR_TYPE_ACCELERATION_STRUCTURE_KHR;
		poolSizes[3].descriptorCount = 1;
		poolSizeCount = 4;
		fprintf(gFILE, "CreateComputeDescriptorPool(): Including acceleration structure in pool (RT enabled)\n");
	}
	...
	vkResult = vkCreateDescriptorPool(vkDevice, &poolInfo, NULL, &vkDescriptorPool_compute);
	if(vkResult != VK_SUCCESS)
	{
		fprintf(gFILE, "CreateComputeDescriptorPool(): vkCreateDescriptorPool() failed with error code %d\n", vkResult);
		return vkResult;
	}
	fprintf(gFILE, "CreateComputeDescriptorPool(): vkCreateDescriptorPool() succeeded with %d pool sizes\n", poolSizeCount);
...
VkResult CreateComputeDescriptorSet(void)
{
	...
	vkResult = vkAllocateDescriptorSets(vkDevice, &allocInfo, &vkDescriptorSet_compute);
	if(vkResult != VK_SUCCESS)
	{
		fprintf(gFILE, "CreateComputeDescriptorSet(): vkAllocateDescriptorSets() failed with error code %d\n", vkResult);
		return vkResult;
	}
	fprintf(gFILE, "CreateComputeDescriptorSet(): vkAllocateDescriptorSets() succeeded\n");

	// Update descriptor set
	// Output storage image
	VkDescriptorImageInfo imageInfo;
	memset(&imageInfo, 0, sizeof(VkDescriptorImageInfo));
	imageInfo.imageView = vkImageView_computeOutput;
	imageInfo.imageLayout = VK_IMAGE_LAYOUT_GENERAL;

	// Uniform buffer
	VkDescriptorBufferInfo bufferInfo;
	memset(&bufferInfo, 0, sizeof(VkDescriptorBufferInfo));
	bufferInfo.buffer = cloudParamsBuffer.vkBuffer;
	bufferInfo.offset = 0;
	bufferInfo.range = sizeof(CloudParamsData);

	// History buffer for temporal reprojection
	VkDescriptorImageInfo historyImageInfo;
	memset(&historyImageInfo, 0, sizeof(VkDescriptorImageInfo));
	historyImageInfo.imageView = vkImageView_historyBuffer;
	historyImageInfo.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
	historyImageInfo.sampler = vkSampler_history;
	...
	descriptorWrites[0].dstSet = vkDescriptorSet_compute;
	...
	descriptorWrites[1].dstSet = vkDescriptorSet_compute;
	...
	descriptorWrites[2].dstSet = vkDescriptorSet_compute;
	descriptorWrites[2].dstBinding = 2;
	descriptorWrites[2].dstArrayElement = 0;
	descriptorWrites[2].descriptorType = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
	descriptorWrites[2].descriptorCount = 1;
	descriptorWrites[2].pImageInfo = &historyImageInfo;

	vkUpdateDescriptorSets(vkDevice, 3, descriptorWrites, 0, NULL);
	fprintf(gFILE, "CreateComputeDescriptorSet(): Descriptor set updated with temporal history buffer\n");

	return vkResult;
}

// Update compute descriptor set with acceleration structure (called after AS creation)
VkResult UpdateComputeDescriptorSetWithAccelerationStructure(void)
{
	if (!bRayTracingSupported || vkAccelerationStructure_TLAS == VK_NULL_HANDLE)
	{
		fprintf(gFILE, "UpdateComputeDescriptorSetWithAccelerationStructure(): RT not available, skipping\n");
		return VK_SUCCESS;
	}

	// Write acceleration structure to descriptor set at binding 3
	VkWriteDescriptorSetAccelerationStructureKHR asWrite = {};
	asWrite.sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET_ACCELERATION_STRUCTURE_KHR;
	asWrite.accelerationStructureCount = 1;
	asWrite.pAccelerationStructures = &vkAccelerationStructure_TLAS;

	VkWriteDescriptorSet descriptorWrite = {};
	descriptorWrite.sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
	descriptorWrite.pNext = &asWrite;
	descriptorWrite.dstSet = vkDescriptorSet_compute;
	...
	fprintf(gFILE, "UpdateComputeDescriptorSetWithAccelerationStructure(): Acceleration structure bound to descriptor set\n");
...
VkResult CreateComputeTextureSampler(void)
{
	...
	// Create sampler for the compute output image to be used by fragment shader
	VkSamplerCreateInfo samplerInfo;
	memset(&samplerInfo, 0, sizeof(VkSamplerCreateInfo));
	samplerInfo.sType = VK_STRUCTURE_TYPE_SAMPLER_CREATE_INFO;
	...
	vkResult = vkCreateSampler(vkDevice, &samplerInfo, NULL, &vkSampler_texture);
	if(vkResult != VK_SUCCESS)
	{
		fprintf(gFILE, "CreateComputeTextureSampler(): vkCreateSampler() failed with error code %d\n", vkResult);
		return vkResult;
	}
	fprintf(gFILE, "CreateComputeTextureSampler(): vkCreateSampler() succeeded\n");

	return vkResult;
}

// Function to update descriptor sets after storage image is recreated during resize
VkResult UpdateDescriptorSetsForNewStorageImage(void)
{
	// Update compute descriptor set with new storage image
	VkDescriptorImageInfo computeImageInfo;
	memset(&computeImageInfo, 0, sizeof(VkDescriptorImageInfo));
	computeImageInfo.imageView = vkImageView_computeOutput;
	computeImageInfo.imageLayout = VK_IMAGE_LAYOUT_GENERAL;

	// History buffer image info for temporal reprojection
	VkDescriptorImageInfo historyImageInfo;
	memset(&historyImageInfo, 0, sizeof(VkDescriptorImageInfo));
	historyImageInfo.imageView = vkImageView_historyBuffer;
	historyImageInfo.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
	...
	VkWriteDescriptorSet computeDescriptorWrites[2];
	memset(computeDescriptorWrites, 0, sizeof(computeDescriptorWrites));

	// Storage image (binding 0)
	computeDescriptorWrites[0].sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
	computeDescriptorWrites[0].dstSet = vkDescriptorSet_compute;
	computeDescriptorWrites[0].dstBinding = 0;
	computeDescriptorWrites[0].dstArrayElement = 0;
	computeDescriptorWrites[0].descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_IMAGE;
	computeDescriptorWrites[0].descriptorCount = 1;
	computeDescriptorWrites[0].pImageInfo = &computeImageInfo;

	// History buffer (binding 2)
	computeDescriptorWrites[1].sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
	computeDescriptorWrites[1].dstSet = vkDescriptorSet_compute;
	computeDescriptorWrites[1].dstBinding = 2;
	computeDescriptorWrites[1].dstArrayElement = 0;
	computeDescriptorWrites[1].descriptorType = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
	computeDescriptorWrites[1].descriptorCount = 1;
	computeDescriptorWrites[1].pImageInfo = &historyImageInfo;

	vkUpdateDescriptorSets(vkDevice, 2, computeDescriptorWrites, 0, NULL);
	fprintf(gFILE, "UpdateDescriptorSetsForNewStorageImage(): Compute descriptor set updated with history buffer\n");

	// Update graphics descriptor set with new storage image for fragment shader sampling
	VkDescriptorImageInfo graphicsImageInfo;
	memset(&graphicsImageInfo, 0, sizeof(VkDescriptorImageInfo));
	graphicsImageInfo.sampler = vkSampler_texture;
	graphicsImageInfo.imageView = vkImageView_computeOutput;
	graphicsImageInfo.imageLayout = VK_IMAGE_LAYOUT_GENERAL;

	VkWriteDescriptorSet graphicsDescriptorWrite;
	memset(&graphicsDescriptorWrite, 0, sizeof(VkWriteDescriptorSet));
	graphicsDescriptorWrite.sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
	graphicsDescriptorWrite.dstSet = vkDescriptorSet;
	graphicsDescriptorWrite.dstBinding = 1; // Texture binding
	...
	vkUpdateDescriptorSets(vkDevice, 1, &graphicsDescriptorWrite, 0, NULL);
	fprintf(gFILE, "UpdateDescriptorSetsForNewStorageImage(): Graphics descriptor set updated\n");

	return VK_SUCCESS;
}

// ==================== END OF COMPUTE SHADER FUNCTIONS ====================

// ============================================================================
// VULKAN RAY TRACING API - ACCELERATION STRUCTURE FUNCTIONS
// These functions create the Bottom Level (BLAS) and Top Level (TLAS)
// acceleration structures required for hardware ray tracing
// ============================================================================

// Helper function to find memory type index
uint32_t findMemoryTypeRT(uint32_t typeFilter, VkMemoryPropertyFlags properties)
{
	for (uint32_t i = 0; i < vkPhysicalDeviceMemoryProperties.memoryTypeCount; i++)
	{
		if ((typeFilter & (1 << i)) &&
...
// Create a buffer with device address capability for ray tracing
VkResult CreateRTBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties,
                        VkBuffer* buffer, VkDeviceMemory* memory, VkDeviceAddress* deviceAddress)
{
	VkResult vkResult = VK_SUCCESS;

	// Include device address bit for RT buffers
	...
	vkResult = vkCreateBuffer(vkDevice, &bufferInfo, NULL, buffer);
	if (vkResult != VK_SUCCESS)
	{
		fprintf(gFILE, "CreateRTBuffer(): vkCreateBuffer() failed\n");
		return vkResult;
	}
	...
	allocInfo.allocationSize = memRequirements.size;
	allocInfo.memoryTypeIndex = findMemoryTypeRT(memRequirements.memoryTypeBits, properties);

	if (allocInfo.memoryTypeIndex == UINT32_MAX)
	{
		fprintf(gFILE, "CreateRTBuffer(): Failed to find suitable memory type\n");
		return VK_ERROR_OUT_OF_DEVICE_MEMORY;
	}

	vkResult = vkAllocateMemory(vkDevice, &allocInfo, NULL, memory);
	if (vkResult != VK_SUCCESS)
	{
		fprintf(gFILE, "CreateRTBuffer(): vkAllocateMemory() failed\n");
		return vkResult;
	}
	...
	// Get device address
	if (deviceAddress != NULL && fpGetBufferDeviceAddressKHR != NULL)
	{
		VkBufferDeviceAddressInfo addressInfo = {};
		addressInfo.sType = VK_STRUCTURE_TYPE_BUFFER_DEVICE_ADDRESS_INFO;
		addressInfo.buffer = *buffer;
		*deviceAddress = fpGetBufferDeviceAddressKHR(vkDevice, &addressInfo);
	}

	return vkResult;
}

// Create ground plane geometry for shadow ray intersection
VkResult CreateGroundPlaneGeometry(void)
{
	VkResult vkResult = VK_SUCCESS;

	// Ground plane vertices - large quad at y=0
	...
	// Ground plane indices - two triangles
	uint32_t groundIndices[] = {
		0, 1, 2,
		0, 2, 3
	};

	VkDeviceSize vertexBufferSize = sizeof(groundVertices);
	VkDeviceSize indexBufferSize = sizeof(groundIndices);

	// Create vertex buffer
	VkDeviceAddress vertexAddr = 0;
	vkResult = CreateRTBuffer(vertexBufferSize,
		VK_BUFFER_USAGE_VERTEX_BUFFER_BIT | VK_BUFFER_USAGE_ACCELERATION_STRUCTURE_BUILD_INPUT_READ_ONLY_BIT_KHR,
		VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
		&vkBuffer_GroundVertices, &vkDeviceMemory_GroundVertices, &vertexAddr);

	if (vkResult != VK_SUCCESS)
	{
		fprintf(gFILE, "CreateGroundPlaneGeometry(): Failed to create vertex buffer\n");
		return vkResult;
	}

	// Copy vertex data
	void* data;
	vkMapMemory(vkDevice, vkDeviceMemory_GroundVertices, 0, vertexBufferSize, 0, &data);
	memcpy(data, groundVertices, vertexBufferSize);
	vkUnmapMemory(vkDevice, vkDeviceMemory_GroundVertices);

	// Create index buffer
	VkDeviceAddress indexAddr = 0;
	vkResult = CreateRTBuffer(indexBufferSize,
		VK_BUFFER_USAGE_INDEX_BUFFER_BIT | VK_BUFFER_USAGE_ACCELERATION_STRUCTURE_BUILD_INPUT_READ_ONLY_BIT_KHR,
		VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
		&vkBuffer_GroundIndices, &vkDeviceMemory_GroundIndices, &indexAddr);

	if (vkResult != VK_SUCCESS)
	{
		fprintf(gFILE, "CreateGroundPlaneGeometry(): Failed to create index buffer\n");
		return vkResult;
	}

	// Copy index data
	vkMapMemory(vkDevice, vkDeviceMemory_GroundIndices, 0, indexBufferSize, 0, &data);
	memcpy(data, groundIndices, indexBufferSize);
	vkUnmapMemory(vkDevice, vkDeviceMemory_GroundIndices);

	fprintf(gFILE, "CreateGroundPlaneGeometry(): Ground plane geometry created successfully\n");
	return vkResult;
}

// Create Bottom Level Acceleration Structure (BLAS) for ground plane
VkResult CreateBottomLevelAccelerationStructure(void)
{
	VkResult vkResult = VK_SUCCESS;

	if (!bRayTracingSupported || fpCreateAccelerationStructureKHR == NULL)
	{
		fprintf(gFILE, "CreateBottomLevelAccelerationStructure(): Ray tracing not supported, skipping\n");
		return VK_SUCCESS;
	}

	// First create the ground geometry
	vkResult = CreateGroundPlaneGeometry();
	if (vkResult != VK_SUCCESS)
	{
		fprintf(gFILE, "CreateBottomLevelAccelerationStructure(): CreateGroundPlaneGeometry() failed\n");
		return vkResult;
	}
	...
	vertexAddrInfo.buffer = vkBuffer_GroundVertices;
	VkDeviceAddress vertexDeviceAddr = fpGetBufferDeviceAddressKHR(vkDevice, &vertexAddrInfo);

	VkBufferDeviceAddressInfo indexAddrInfo = {};
	indexAddrInfo.sType = VK_STRUCTURE_TYPE_BUFFER_DEVICE_ADDRESS_INFO;
	indexAddrInfo.buffer = vkBuffer_GroundIndices;
	VkDeviceAddress indexDeviceAddr = fpGetBufferDeviceAddressKHR(vkDevice, &indexAddrInfo);

	// Define geometry - triangles for ground plane
	VkAccelerationStructureGeometryTrianglesDataKHR triangles = {};
	triangles.sType = VK_STRUCTURE_TYPE_ACCELERATION_STRUCTURE_GEOMETRY_TRIANGLES_DATA_KHR;
	triangles.vertexFormat = VK_FORMAT_R32G32B32_SFLOAT;
	triangles.vertexData.deviceAddress = vertexDeviceAddr;
	triangles.vertexStride = sizeof(float) * 3;
	triangles.maxVertex = 3;
	triangles.indexType = VK_INDEX_TYPE_UINT32;
	triangles.indexData.deviceAddress = indexDeviceAddr;
	...
	uint32_t primitiveCount = 2; // Two triangles for ground quad
	VkAccelerationStructureBuildSizesInfoKHR buildSizesInfo = {};
	buildSizesInfo.sType = VK_STRUCTURE_TYPE_ACCELERATION_STRUCTURE_BUILD_SIZES_INFO_KHR;
	fpGetAccelerationStructureBuildSizesKHR(vkDevice, VK_ACCELERATION_STRUCTURE_BUILD_TYPE_DEVICE_KHR,
		&buildGeometryInfo, &primitiveCount, &buildSizesInfo);

	fprintf(gFILE, "CreateBottomLevelAccelerationStructure(): BLAS size = %llu bytes\n",
		buildSizesInfo.accelerationStructureSize);

	// Create BLAS buffer
	vkResult = CreateRTBuffer(buildSizesInfo.accelerationStructureSize,
		VK_BUFFER_USAGE_ACCELERATION_STRUCTURE_STORAGE_BIT_KHR,
		VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
		&vkBuffer_BLAS, &vkDeviceMemory_BLAS, NULL);

	if (vkResult != VK_SUCCESS)
	{
		fprintf(gFILE, "CreateBottomLevelAccelerationStructure(): Failed to create BLAS buffer\n");
		return vkResult;
	}
	...
	createInfo.buffer = vkBuffer_BLAS;
	createInfo.size = buildSizesInfo.accelerationStructureSize;
	createInfo.type = VK_ACCELERATION_STRUCTURE_TYPE_BOTTOM_LEVEL_KHR;

	vkResult = fpCreateAccelerationStructureKHR(vkDevice, &createInfo, NULL, &vkAccelerationStructure_BLAS);
	if (vkResult != VK_SUCCESS)
	{
		fprintf(gFILE, "CreateBottomLevelAccelerationStructure(): fpCreateAccelerationStructureKHR() failed\n");
		return vkResult;
	}

	// Create scratch buffer for building
	VkDeviceAddress scratchAddr = 0;
	vkResult = CreateRTBuffer(buildSizesInfo.buildScratchSize,
		VK_BUFFER_USAGE_STORAGE_BUFFER_BIT,
		VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
		&vkBuffer_ASScratch, &vkDeviceMemory_ASScratch, &scratchAddr);

	if (vkResult != VK_SUCCESS)
	{
		fprintf(gFILE, "CreateBottomLevelAccelerationStructure(): Failed to create scratch buffer\n");
		return vkResult;
	}

	// Update build info with destination and scratch
	buildGeometryInfo.mode = VK_BUILD_ACCELERATION_STRUCTURE_MODE_BUILD_KHR;
	buildGeometryInfo.dstAccelerationStructure = vkAccelerationStructure_BLAS;
	...
	fpCmdBuildAccelerationStructuresKHR(cmdBuffer, 1, &buildGeometryInfo, pBuildRangeInfos);
	...
	blasAddrInfo.accelerationStructure = vkAccelerationStructure_BLAS;
	vkDeviceAddress_BLAS = fpGetAccelerationStructureDeviceAddressKHR(vkDevice, &blasAddrInfo);

	fprintf(gFILE, "CreateBottomLevelAccelerationStructure(): BLAS created successfully, address = 0x%llx\n",
		vkDeviceAddress_BLAS);

	return vkResult;
}

// Create Top Level Acceleration Structure (TLAS) that references the BLAS
VkResult CreateTopLevelAccelerationStructure(void)
{
	VkResult vkResult = VK_SUCCESS;

	if (!bRayTracingSupported || vkAccelerationStructure_BLAS == VK_NULL_HANDLE)
	{
		fprintf(gFILE, "CreateTopLevelAccelerationStructure(): Ray tracing not supported or BLAS not created, skipping\n");
		return VK_SUCCESS;
	}

	// Create instance for TLAS - references our BLAS
	...
	instance.accelerationStructureReference = vkDeviceAddress_BLAS;

	// Create instance buffer
	VkDeviceAddress instanceAddr = 0;
	vkResult = CreateRTBuffer(sizeof(VkAccelerationStructureInstanceKHR),
		VK_BUFFER_USAGE_ACCELERATION_STRUCTURE_BUILD_INPUT_READ_ONLY_BIT_KHR,
		VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
		&vkBuffer_TLASInstances, &vkDeviceMemory_TLASInstances, &instanceAddr);

	if (vkResult != VK_SUCCESS)
	{
		fprintf(gFILE, "CreateTopLevelAccelerationStructure(): Failed to create instance buffer\n");
		return vkResult;
	}

	// Copy instance data
	void* data;
	vkMapMemory(vkDevice, vkDeviceMemory_TLASInstances, 0, sizeof(VkAccelerationStructureInstanceKHR), 0, &data);
	memcpy(data, &instance, sizeof(VkAccelerationStructureInstanceKHR));
	vkUnmapMemory(vkDevice, vkDeviceMemory_TLASInstances);
	...
	fprintf(gFILE, "CreateTopLevelAccelerationStructure(): TLAS size = %llu bytes\n",
		buildSizesInfo.accelerationStructureSize);
	...
	vkResult = CreateRTBuffer(buildSizesInfo.accelerationStructureSize,
		VK_BUFFER_USAGE_ACCELERATION_STRUCTURE_STORAGE_BIT_KHR,
		VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
		&vkBuffer_TLAS, &vkDeviceMemory_TLAS, NULL);

	if (vkResult != VK_SUCCESS)
	{
		fprintf(gFILE, "CreateTopLevelAccelerationStructure(): Failed to create TLAS buffer\n");
		return vkResult;
	}
	...
	VkAccelerationStructureCreateInfoKHR createInfo = {};
	createInfo.sType = VK_STRUCTURE_TYPE_ACCELERATION_STRUCTURE_CREATE_INFO_KHR;
	createInfo.buffer = vkBuffer_TLAS;
	createInfo.size = buildSizesInfo.accelerationStructureSize;
	createInfo.type = VK_ACCELERATION_STRUCTURE_TYPE_TOP_LEVEL_KHR;

	vkResult = fpCreateAccelerationStructureKHR(vkDevice, &createInfo, NULL, &vkAccelerationStructure_TLAS);
	if (vkResult != VK_SUCCESS)
	{
		fprintf(gFILE, "CreateTopLevelAccelerationStructure(): fpCreateAccelerationStructureKHR() failed\n");
		return vkResult;
	}

	// Create scratch buffer (reuse if exists, or create new)
	...
	if (vkResult != VK_SUCCESS)
	{
		fprintf(gFILE, "CreateTopLevelAccelerationStructure(): Failed to create scratch buffer\n");
		return vkResult;
	}

	// Build TLAS
	buildGeometryInfo.mode = VK_BUILD_ACCELERATION_STRUCTURE_MODE_BUILD_KHR;
	buildGeometryInfo.dstAccelerationStructure = vkAccelerationStructure_TLAS;
	buildGeometryInfo.scratchData.deviceAddress = scratchAddr;
	...
	vkFreeMemory(vkDevice, scratchMemory, NULL);
	...
	tlasAddrInfo.accelerationStructure = vkAccelerationStructure_TLAS;
	vkDeviceAddress_TLAS = fpGetAccelerationStructureDeviceAddressKHR(vkDevice, &tlasAddrInfo);

	fprintf(gFILE, "CreateTopLevelAccelerationStructure(): TLAS created successfully, address = 0x%llx\n",
		vkDeviceAddress_TLAS);

	return vkResult;
}

// Main function to create all ray tracing acceleration structures
VkResult CreateRayTracingAccelerationStructures(void)
{
	VkResult vkResult = VK_SUCCESS;
	...
	if (!bRayTracingSupported)
	{
		fprintf(gFILE, "CreateRayTracingAccelerationStructures(): Ray tracing not supported\n");
		return VK_ERROR_INITIALIZATION_FAILED;
	}

	fprintf(gFILE, "CreateRayTracingAccelerationStructures(): Creating acceleration structures...\n");

	// Create BLAS (bottom level - geometry)
	vkResult = CreateBottomLevelAccelerationStructure();
	if (vkResult != VK_SUCCESS)
	{
		fprintf(gFILE, "CreateRayTracingAccelerationStructures(): BLAS creation failed\n");
		return vkResult;
	}

	// Create TLAS (top level - instances)
	vkResult = CreateTopLevelAccelerationStructure();
	if (vkResult != VK_SUCCESS)
	{
		fprintf(gFILE, "CreateRayTracingAccelerationStructures(): TLAS creation failed\n");
		return vkResult;
	}

	fprintf(gFILE, "CreateRayTracingAccelerationStructures(): All acceleration structures created successfully\n");
	return vkResult;
}
...
VkResult CreateDescriptorSet(void)
{
	...
	*/
	...
	// Use compute shader output image instead of loaded texture
	vkDescriptorImageInfo.imageView = vkImageView_computeOutput;
	// Use GENERAL layout since compute shader writes to this image
...
VkResult CreateSemaphores(void)
{
	...
	*/
	...
	/*
	18_3. Create per-frame semaphores for proper synchronization with frames in flight.
	We create MAX_FRAMES_IN_FLIGHT semaphores (independent of swapchain image count).
	*/
	...
	for(int i = 0; i < MAX_FRAMES_IN_FLIGHT; i++)
	{
		...
		vkResult = vkCreateSemaphore(vkDevice, &vkSemaphoreCreateInfo, NULL, &vkSemaphore_ImageAvailable_array[i]);
		if (vkResult != VK_SUCCESS)
		{
			fprintf(gFILE, "CreateSemaphores(): vkCreateSemaphore() failed for vkSemaphore_ImageAvailable_array[%d]\n", i);
			return vkResult;
		}
		else
		{
			fprintf(gFILE, "CreateSemaphores(): vkCreateSemaphore() succedded for vkSemaphore_ImageAvailable_array[%d]\n", i);
		}

		vkResult = vkCreateSemaphore(vkDevice, &vkSemaphoreCreateInfo, NULL, &vkSemaphore_RenderComplete_array[i]);
		if (vkResult != VK_SUCCESS)
		{
			fprintf(gFILE, "CreateSemaphores(): vkCreateSemaphore() failed for vkSemaphore_RenderComplete_array[%d]\n", i);
			return vkResult;
		}
		else
		{
			fprintf(gFILE, "CreateSemaphores(): vkCreateSemaphore() succedded for vkSemaphore_RenderComplete_array[%d]\n", i);
		}
	}
...
VkResult CreateFences(void)
{
	...
	*/
	...
	/*
	18_5. Create per-frame fences (MAX_FRAMES_IN_FLIGHT) for synchronization.
	*/
	for(int i = 0; i < MAX_FRAMES_IN_FLIGHT; i++)
	{
		//https://registry.khronos.org/vulkan/specs/latest/man/html/vkCreateFence.html
		vkResult = vkCreateFence(vkDevice, &vkFenceCreateInfo, NULL, &vkFence_InFlight_array[i]);
		if (vkResult != VK_SUCCESS)
		{
			fprintf(gFILE, "CreateFences(): vkCreateFence() failed at %d iteration\n", i);
			return vkResult;
		}
		else
		{
	...
	/*
	18_6. Allocate and initialize the images-in-flight tracking array.
	This tracks which fence is currently using each swapchain image.
	Initially all are VK_NULL_HANDLE (no fence using any image).
	*/
	vkFence_ImagesInFlight_array = (VkFence*)malloc(sizeof(VkFence) * swapchainImageCount);
	if(vkFence_ImagesInFlight_array == NULL)
	{
		fprintf(gFILE, "CreateFences(): malloc() failed for vkFence_ImagesInFlight_array\n");
		return VK_ERROR_OUT_OF_HOST_MEMORY;
	}

	// Initialize all to VK_NULL_HANDLE - no images are in flight initially
	for(uint32_t i = 0; i < swapchainImageCount; i++)
	{
		vkFence_ImagesInFlight_array[i] = VK_NULL_HANDLE;
	}
	fprintf(gFILE, "CreateFences(): vkFence_ImagesInFlight_array initialized for %d images\n", swapchainImageCount);
	...
	*/
	for(uint32_t i =0; i< swapchainImageCount; i++)
	{
		...
		*/
		...
		// ============ COMPUTE SHADER DISPATCH FOR PROCEDURAL CLOUDS ============
		// Bind compute pipeline
		vkCmdBindPipeline(vkCommandBuffer_array[i], VK_PIPELINE_BIND_POINT_COMPUTE, vkPipeline_compute);

		// Bind compute descriptor set
		vkCmdBindDescriptorSets(vkCommandBuffer_array[i], VK_PIPELINE_BIND_POINT_COMPUTE, vkPipelineLayout_compute, 0, 1, &vkDescriptorSet_compute, 0, NULL);

		// Dispatch compute shader - workgroup size is 8x8, so dispatch enough groups to cover the screen
		uint32_t dispatchX = (vkExtent2D_SwapChain.width + 7) / 8;
		uint32_t dispatchY = (vkExtent2D_SwapChain.height + 7) / 8;
		vkCmdDispatch(vkCommandBuffer_array[i], dispatchX, dispatchY, 1);

		// Memory barrier to ensure compute shader writes are visible to fragment shader
		VkImageMemoryBarrier computeToFragmentBarrier;
		memset(&computeToFragmentBarrier, 0, sizeof(VkImageMemoryBarrier));
		computeToFragmentBarrier.sType = VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER;
		computeToFragmentBarrier.oldLayout = VK_IMAGE_LAYOUT_GENERAL;
		computeToFragmentBarrier.newLayout = VK_IMAGE_LAYOUT_GENERAL;
		computeToFragmentBarrier.srcQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED;
		computeToFragmentBarrier.dstQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED;
		computeToFragmentBarrier.image = vkImage_computeOutput;
		computeToFragmentBarrier.subresourceRange.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT;
		computeToFragmentBarrier.subresourceRange.baseMipLevel = 0;
		computeToFragmentBarrier.subresourceRange.levelCount = 1;
		computeToFragmentBarrier.subresourceRange.baseArrayLayer = 0;
		computeToFragmentBarrier.subresourceRange.layerCount = 1;
		...
		vkCmdPipelineBarrier(vkCommandBuffer_array[i],
			...
			0, 0, NULL, 0, NULL, 1, &computeToFragmentBarrier);

		// ============ TEMPORAL REPROJECTION - Copy output to history buffer ============
		// Transition history buffer to transfer destination
		VkImageMemoryBarrier historyToTransferBarrier;
		memset(&historyToTransferBarrier, 0, sizeof(VkImageMemoryBarrier));
		historyToTransferBarrier.sType = VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER;
		historyToTransferBarrier.oldLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
		historyToTransferBarrier.newLayout = VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL;
		historyToTransferBarrier.srcQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED;
		historyToTransferBarrier.dstQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED;
		historyToTransferBarrier.image = vkImage_historyBuffer;
		historyToTransferBarrier.subresourceRange.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT;
		historyToTransferBarrier.subresourceRange.baseMipLevel = 0;
		historyToTransferBarrier.subresourceRange.levelCount = 1;
		historyToTransferBarrier.subresourceRange.baseArrayLayer = 0;
		historyToTransferBarrier.subresourceRange.layerCount = 1;
		historyToTransferBarrier.srcAccessMask = VK_ACCESS_SHADER_READ_BIT;
		historyToTransferBarrier.dstAccessMask = VK_ACCESS_TRANSFER_WRITE_BIT;

		// Transition compute output to transfer source
		VkImageMemoryBarrier outputToTransferBarrier;
		memset(&outputToTransferBarrier, 0, sizeof(VkImageMemoryBarrier));
		outputToTransferBarrier.sType = VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER;
		outputToTransferBarrier.oldLayout = VK_IMAGE_LAYOUT_GENERAL;
		outputToTransferBarrier.newLayout = VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL;
		...
		outputToTransferBarrier.image = vkImage_computeOutput;
		outputToTransferBarrier.subresourceRange.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT;
		...
		outputToTransferBarrier.srcAccessMask = VK_ACCESS_SHADER_WRITE_BIT;
		outputToTransferBarrier.dstAccessMask = VK_ACCESS_TRANSFER_READ_BIT;

		VkImageMemoryBarrier preTransferBarriers[2] = {historyToTransferBarrier, outputToTransferBarrier};
		vkCmdPipelineBarrier(vkCommandBuffer_array[i],
			VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT | VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT,
			VK_PIPELINE_STAGE_TRANSFER_BIT,
			0, 0, NULL, 0, NULL, 2, preTransferBarriers);

		// Copy compute output to history buffer
		...
		copyRegion.extent.width = vkExtent2D_SwapChain.width;
		copyRegion.extent.height = vkExtent2D_SwapChain.height;
		copyRegion.extent.depth = 1;

		vkCmdCopyImage(vkCommandBuffer_array[i],
			vkImage_computeOutput, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
			vkImage_historyBuffer, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
			1, &copyRegion);

		// Transition images back to their working layouts
		VkImageMemoryBarrier historyBackBarrier;
		memset(&historyBackBarrier, 0, sizeof(VkImageMemoryBarrier));
		historyBackBarrier.sType = VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER;
		historyBackBarrier.oldLayout = VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL;
		historyBackBarrier.newLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
		historyBackBarrier.srcQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED;
		historyBackBarrier.dstQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED;
		historyBackBarrier.image = vkImage_historyBuffer;
		historyBackBarrier.subresourceRange.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT;
		historyBackBarrier.subresourceRange.baseMipLevel = 0;
		historyBackBarrier.subresourceRange.levelCount = 1;
		historyBackBarrier.subresourceRange.baseArrayLayer = 0;
		historyBackBarrier.subresourceRange.layerCount = 1;
		historyBackBarrier.srcAccessMask = VK_ACCESS_TRANSFER_WRITE_BIT;
		historyBackBarrier.dstAccessMask = VK_ACCESS_SHADER_READ_BIT;

		VkImageMemoryBarrier outputBackBarrier;
		memset(&outputBackBarrier, 0, sizeof(VkImageMemoryBarrier));
		...
		outputBackBarrier.image = vkImage_computeOutput;
		...
		outputBackBarrier.dstAccessMask = VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT;

		VkImageMemoryBarrier postTransferBarriers[2] = {historyBackBarrier, outputBackBarrier};
		vkCmdPipelineBarrier(vkCommandBuffer_array[i],
			VK_PIPELINE_STAGE_TRANSFER_BIT,
			VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT | VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT,
			0, 0, NULL, 0, NULL, 2, postTransferBarriers);
		// ============ END OF TEMPORAL REPROJECTION COPY ============

		// ============ END OF COMPUTE SHADER DISPATCH ============
